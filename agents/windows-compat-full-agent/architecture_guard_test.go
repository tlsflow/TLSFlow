...
func TestServiceScriptsUseGoBundleLayout(t *testing.T) {
	for _, path := range []string{"install-service.ps1", "upgrade-service.ps1"} {
		content, err := os.ReadFile(path)
		if err != nil {
			t.Fatalf("读取服务脚本失败 path=%s: %v", path, err)
		}
		text := strings.ToLower(string(content))
		if !strings.Contains(text, "web-iis\\web-iis-agent-side-plugin.exe") {
			t.Errorf("服务脚本必须读取 Release Bundle 的 Go IIS Plugin 路径: %s", path)
		}
		if strings.Contains(text, "windowscompatibilityagent.exe.config") || strings.Contains(text, "bin\\release") {
			t.Errorf("服务脚本包含旧 .NET 资源路径: %s", path)
		}
	}
}
