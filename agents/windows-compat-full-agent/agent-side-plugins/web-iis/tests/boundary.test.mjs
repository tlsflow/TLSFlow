...
const sideSource = readFileSync(join(testsDirectory, '..', 'src', 'IisAgentSidePlugin.cs'), 'utf8');
const runnerSource = readFileSync(join(root, 'backend', 'src', 'modules', 'plugins', 'builtin-plugins', 'web-iis', 'runtime', 'index.js'), 'utf8');

test('IIS 专用 API 只存在于独立 Agent-side Plugin 进程', () => {
  assert.match(sideSource, /Microsoft\.Web\.Administration/);
  ...
  for (const directory of [
    join(root, 'agents', 'windows-compat-full-agent', 'src'),
    join(root, 'agents', 'windows-go-full-agent'),
    join(root, 'agents', 'linux-go-full-agent'),
  ]) {
...
function codeFiles(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (/\.(cs|go|csproj|xml)$/.test(entry.name)) files.push(path);
  }
  return files;
}
