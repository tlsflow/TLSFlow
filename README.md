...
支持 IIS、NGINX、Tomcat、Citrix ADC 等企业应用，兼容隔离网络、旧系统、云平台和网络设备。
...
| 功能 | 说明 |
| --- | --- |
| **把证书管起来** | 每张证书的到期时间、版本、部署服务器和绑定应用全部记录，历史版本保留，随时可查。 |
| **适配各种环境** | Windows/Linux 服务器装 Agent，隔离区走 Gateway 中转，老旧设备用工作流推送，支持 IIS、NGINX、Tomcat 和负载均衡。 |
| **更新前先备份** | 更新前自动备份原配置，更新后立即验证 HTTPS；发现问题自动恢复原配置。 |
| **常用与特殊应用都能接管** | 常见应用通过插件开箱即用，特殊设备可用工作流编排文件传输、命令执行和 API 调用。 |
| **自己签发证书** | 内网环境可使用内置 CA 签发证书，外部购买的证书也可导入统一管理并检查证书链。 |
| **到期提醒和失败告警** | 对到期、部署失败、Agent 离线和证书指纹异常进行告警，支持微信、钉钉、邮件、Webhook 等通知及运营报表。 |
...
- **证书更新故障自动回退**：更新前自动备份，验证失败马上恢复，避免业务挂起后再手忙脚乱地回滚配置。
- **常用与特殊应用都能接管**：IIS、NGINX 等常见应用通过插件直接使用，冷门设备和特殊流程可以自行编排更新步骤。
...
- **应用类型复杂**：IIS、NGINX、Tomcat、负载均衡、网络设备和云平台并存，手工维护难以持续；
...
| 能力域 | 能解决的问题 |
| --- | --- |
| 身份、RBAC 与对象授权 | 谁能查看、编辑、审批或执行某一类证书和目标 |
| Secret、审批与审计 | 凭据如何安全保存、按需使用并留下责任链 |
| 证书资产与 CA | 证书版本、格式、制品、信任域和签发过程如何统一管理 |
| 设备、Agent 与 Gateway | 如何连接主机、发现能力并覆盖隔离网络 |
| 应用资产与证书绑定 | 一张证书实际服务哪些业务、站点和目标 |
| 插件平台与兼容目录 | 高频产品如何标准化接入，兼容范围如何声明 |
| 工作流 DSL 与模板 | 长尾设备和内部 API 如何低成本接入 |
| 部署计划与执行编排 | 预检、快照、锁、步骤追踪、验证和回滚如何串联 |
| 监控、自动化与报表 | 到期、过期、漂移、离线、失败和运营指标如何持续跟踪 |
...
| 数据持久化 | 标准部署使用 PostgreSQL；单机评估使用 PGlite |
