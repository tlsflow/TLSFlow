...
export const internalCaApi = {
  ...
  deleteTrustDomain: (trustDomainId: string) => remove(`/api/v1/ca-trust-domains/${encodeURIComponent(trustDomainId)}`),
  listAuthorities: () => getList('/api/v1/certificate-authorities'),
  updateAuthority: (authorityId: string, body: ApiBody) => patch(`/api/v1/certificate-authorities/${encodeURIComponent(authorityId)}`, body),
  ...
  deleteProfile: (profileId: string) => remove(`/api/v1/certificate-profiles/${encodeURIComponent(profileId)}`),
