...
export default {
  ...
  internalCa: {
    ...
    adcsAgent: { title: 'Microsoft AD CS 连接', description: '每套 AD CS CA 使用独立 Provider 和 Agent，可继续添加多套域、根 CA 或签发 CA。', providerCount: '当前已配置 {count} 套 Microsoft AD CS Provider', addProvider: '添加 AD CS', wizardTitle: '添加 Microsoft AD CS', wizardDescription: '创建独立 Provider，并生成该 AD CS 服务器专用的一次性安装命令。', multiProviderTitle: '支持管理多套 AD CA', multiProviderDescription: '每次向导都会创建新的 Provider，不会覆盖已有 AD CS 连接。', providerName: 'AD CS Provider 名称', createCommand: '创建并生成安装命令', commandReadyTitle: '安装命令已生成', commandReadyDescription: '已创建 {name}，请在对应 AD CS 服务器的管理员 PowerShell 中执行以下命令。', expiresAt: '命令到期时间：{time}', deleteProvider: '删除 Provider', deletingProvider: '删除中…', deleteProviderRisk: '将删除该 Provider 及其未完成的注册令牌、Agent 节点、任务和能力记录。已绑定证书机构的 Provider 不允许删除。', deleteProviderBlocked: '该 Provider 已被证书机构使用，必须先迁移或移除对应证书机构。', defaultProviderName: 'Microsoft AD CS Agent', defaultProviderNameIndexed: 'Microsoft AD CS {index}' },
