...
export default {
  ...
  automations: {
    ...
    eventSources: { external_source: '外部来源', manual_import: '手工导入' },
  ...
  internalCa: {
    ...
    requests: { recordsTitle: '证书申请记录', columns: { commonName: 'Common Name', applicationAssetId: '应用资产 ID', updatedAt: '更新时间', actions: '操作' }, empty: '暂无证书申请记录。', modalTitle: '新建证书申请', modalDescription: '填写证书申请信息，提交后将进入审批与签发流程。' },
    profiles: { recordsTitle: '证书 Profile 记录', columns: { securityDomain: '安全域', versionCount: '版本数' }, empty: '暂无证书 Profile 记录。', modalTitle: '新建证书 Profile', modalDescription: '定义证书签发规则与约束，包括有效期、DNS 后缀和审批要求。' },
    ...
    backendSummary: { createAndIssue: '可创建和签发证书', requestPublicCertificates: '可申请公共证书', external: '需接入外部执行器', localVerified: '本地验证通过', remoteVerified: '连接验证通过', unverified: '尚未验证' },
    ...
    wizard: { title: '添加证书颁发机构', description: '使用内置签发方式，再逐步配置 CA 参数和安全边界。', stepsAria: 'CA 创建步骤', entryStep: '选择方式', backendStep: '配置后端', parentStep: '选择父 CA', authorityStep: '配置 CA', reviewStep: '确认创建', completed: '已完成', inProgress: '进行中', pending: '待填写', entryEyebrow: '第一步', entryTitle: '这套 CA 由谁负责签发？', entryDescription: '当前版本仅支持由 GCAC 服务内置执行面完成签发。', recommended: '推荐起步', builtinTitle: '直接创建 CA', builtinDescription: '由当前 GCAC 服务内置的通用证书签发执行面完成。', builtinFeature1: '无需部署额外节点', builtinFeature2: '适合开发和中小规模内部环境', backendEyebrow: '签发后端', builtinBackendTitle: '使用 GCAC 内置签发后端', builtinBackendDescription: '系统自动创建或复用租户内置执行后端，用户只需要配置 CA。', builtinAutomaticTitle: '无需单独创建执行后端', builtinAutomaticDescription: '创建 CA 时系统会自动确保内置签发执行后端存在，并绑定到当前 CA。', authorityEyebrow: '证书机构', rootConfigurationTitle: '配置根 CA', rootConfigurationDescription: '定义新的根信任边界、名称、主题和是否启用中间 CA。', intermediateConfigurationTitle: '配置中间 CA', intermediateConfigurationDescription: '先选择父根 CA，再配置承担日常签发的中间证书颁发机构。', advancedSubjectTitle: '高级证书主题设置', commonNameHelp: '写入 CA 证书主题，用于证书链识别，不是域名。', builtinSecurityNote: '软件私钥由 GCAC SecretService 托管，不等同于不可导出 HSM 密钥。', reviewEyebrow: '最终确认', reviewTitle: '检查信任边界与签发方式', reviewDescription: '确认 CA 名称、信任域、签发后端和风险提示后再创建。', builtinProviderName: 'GCAC 内置签发后端', rootTitle: '根 CA', rootDescription: '创建新的独立根信任锚点，并可同时创建首个中间 CA。', intermediateTitle: '中间 CA', intermediateDescription: '挂载到已有根 CA 下承担日常签发，不创建新的根信任边界。', noWarnings: '未发现额外的拓扑风险警告。' },
