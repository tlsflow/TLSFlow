...
export default {
  ...
  internalCa: {
    ...
    profiles: { recordsTitle: '证书 Profile 记录', columns: { securityDomain: '安全域', versionCount: '版本数' }, empty: '暂无证书 Profile 记录。', modalTitle: '新建证书 Profile', modalDescription: '创建可复用的 HTTPS 证书规则；创建后在证书机构中关联并生效。', applicableDomains: '适用域名', applicableDomainsPlaceholder: '*.example.com, *.example.org', applicableDomainsHint: '留空表示不限制；填写的域名同时用于 Profile 匹配和 DNS 后缀校验。' },
