...
export default {
  ...
  automations: {
    ...
    fields: { name: '名称', description: '说明', trigger: '触发方式', cron: 'Cron 表达式', timeZone: '时区', expiresWithinDays: '到期天数范围', environments: '目标环境（逗号分隔）', maxTargets: '单次最大目标数', concurrency: '并发数', failureCount: '失败数量阈值', requireDryRun: '执行前必须完成 Dry Run', requireApproval: '执行前必须审批', startedAt: '开始时间', finishedAt: '结束时间', failureStage: '失败阶段', parentRun: '父运行' },
    ...
    editor: { createTitle: '新建自动化', editTitle: '编辑自动化', description: '配置触发方式、目标选择条件、执行动作和安全护栏。' },
