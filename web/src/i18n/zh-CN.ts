...
export default {
  ...
  internalCa: {
    ...
    sections: { trustDomain: '创建 CA 信任域', provider: 'CA Provider', providerSettings: 'CA Provider 配置', authorityWizard: 'CA 创建向导', authorityOverview: '证书机构架构', authorityOverviewDescription: '每张卡片代表一个根信任锚点，选择卡片可查看其下级签发架构。', caArchitecture: 'CA 层级架构', riskSummary: '安全决策摘要', profile: '创建证书 Profile', request: '创建应用证书申请', revocation: '创建吊销任务', trust: '创建信任分发任务', remediation: '整改预览' },
    fields: { name: '名称', code: '唯一代码', purpose: '用途', isolationLevel: '隔离等级', defaultTrustDomain: '设为默认信任域', trustDomain: 'CA 信任域', parentAuthority: '父根 CA', authorityType: '证书机构类型', providerType: 'Provider 类型', deploymentMode: '部署模式', platform: '运行平台', provider: 'CA Provider', commonName: 'Common Name', securityDomain: '安全域', topology: 'CA 拓扑', dnsSuffixes: '允许的 DNS 后缀', validityDays: '最大有效期（天）', renewalDays: '提前续期（天）', requireApproval: '签发前需要审批', applicationAssetId: '应用资产 ID', authority: '证书机构', profileVersionId: 'Profile 版本 ID', sans: 'SAN 列表', custodyMode: '密钥托管模式', certificateVersionId: '证书版本 ID', reason: '吊销原因', targetIds: '目标 ID 列表' },
    actions: { refresh: '刷新', addAuthority: '添加 CA', addIntermediate: '添加中间 CA', previous: '上一步', next: '下一步', createTrustDomain: '创建信任域', createProvider: '创建 Provider', previewRisk: '预览风险', createAuthority: '创建 CA', createProfile: '创建 Profile', createRequest: '提交申请', approve: '审批通过', retry: '重试', queryResult: '查询结果', scanRenewals: '扫描到期续期', createRevocation: '创建吊销任务', createTrust: '创建信任分发', previewRemediation: '预览整改' },
    placeholders: { dnsSuffixes: 'example.com, office.example.com', sans: 'oa.example.com, 10.0.0.10' },
    messages: { loadFailed: '内部 CA 数据加载失败。', actionFailed: '操作失败，请检查输入、权限和审批状态。', noIntermediate: '该根 CA 尚未配置中间证书颁发机构。', noRootAuthority: '尚未配置根 CA', noRootAuthorityDescription: '添加根 CA 以建立第一套独立信任架构。', trustDomainCreated: 'CA 信任域已创建。', providerCreated: 'CA Provider 已创建。', authorityCreated: '证书机构已创建。', profileCreated: '证书 Profile 已创建。', requestCreated: '证书申请已提交。', requestApproved: '证书申请已审批。', requestRetried: '证书签发已重试。', requestQueried: '远程签发结果已刷新。', renewalScanned: '续期扫描已完成。', revocationCreated: '吊销任务已创建并等待审批。', revocationApproved: '证书吊销已审批。', trustCreated: '信任分发任务已创建并等待审批。', trustApproved: '信任分发已审批。' },
    metrics: { nodes: 'CA Node', renewals: '续期任务', revocations: '吊销任务', trust: '信任分发', totalRisks: '风险总数', critical: '严重风险', affectedAssets: '受影响应用资产' },
    labels: { rootAuthority: '根证书颁发机构', intermediateAuthority: '中间证书颁发机构', intermediateCount: '{count} 个中间 CA', expiresAt: '到期时间：{time}', defaultTrustDomain: '默认信任域', independentTrustDomain: '独立根信任边界', trustDomainCount: '{count} 个 CA 信任域', versionCount: '{count} 个版本', assetCount: '{count} 个应用资产', requestCount: '将创建 {count} 个独立证书申请' },
    wizard: { title: '添加证书颁发机构', description: '创建新的根信任架构，或在已有根 CA 下添加中间证书颁发机构。', stepsAria: 'CA 创建步骤', step1: '选择类型', step2: '填写配置', step3: '确认风险', rootTitle: '根 CA', rootDescription: '创建新的独立根信任锚点，并可同时创建首个中间 CA。', intermediateTitle: '中间 CA', intermediateDescription: '挂载到已有根 CA 下承担日常签发，不创建新的根信任边界。', noWarnings: '未发现额外的拓扑风险警告。' },
