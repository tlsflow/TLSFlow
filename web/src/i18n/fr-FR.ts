...
export default {
  ...
  tasks: {
    ...
    values: { system: 'Système', externalApi: 'API externe', empty: 'Aucun enregistrement', none: 'Aucune' },
    agentUpdate: {
    ...
    pluginRefresh: {
      ...
      events: { created: "La tâche a été créée et attend son traitement.", claimed: "La tâche a été attribuée à un processeur en arrière-plan.", started: "Lecture du catalogue de plugins démarrée.", progress: "Préparation des versions et synchronisation des nœuds en cours.", retryScheduled: "Le traitement n'est pas terminé ; une nouvelle tentative a été planifiée.", waitingResult: "En attente des résultats des nœuds.", awaitingConfirmation: "Le résultat doit être confirmé.", cancelRequested: "Une demande d'annulation a été reçue.", expired: "La tâche a expiré.", cancelled: "La tâche d'actualisation a été annulée.", succeeded: "Actualisation terminée : {versions} versions et {projected} nœuds synchronisés.", failed: "Échec de l'actualisation : {reason}" },
      versionStatus: { added: 'Ajoutée', enabled: 'Activée', disabled: 'Désactivée', other: 'Autre état' }
    },
    approval: {
  ...
  automations: {
    ...
    externalApi: { executionModeLabel: 'Mode d’exécution externe', executionModeAria: 'Mode d’exécution externe', keyTitle: 'Clé API externe', keyDescription: 'Cette clé reste visible sur la page de l’automatisation et peut être copiée à tout moment.', keyNotice: 'Envoyez-la dans l’en-tête X-Automation-API-Key.', keyStatusPending: 'Générée après l’enregistrement et l’activation', keyStatusActive: 'Générée', keyStatusUnavailable: 'Générée (la clé complète n’est pas visible sur cette page)', keyEditorDescription: 'La clé complète reste visible sur la page et peut être copiée à tout moment. L’actualisation invalide immédiatement l’ancienne clé.', keyValueLabel: 'Clé API', keyValueAria: 'Clé API externe', keyUnavailableValue: 'La clé complète n’est pas visible sur cette page', keyUnavailable: 'Cette page ne contient pas la clé complète. Cliquez sur « Actualiser la clé » pour en générer une nouvelle.', rotate: 'Actualiser la clé', rotating: 'Actualisation', rotateNotice: 'L’actualisation invalide immédiatement l’ancienne clé.', mode: 'Mode d’exécution : {mode}', copy: 'Copier la clé', copied: 'Copiée', copyAria: 'Copier la clé API', copyCurlAria: 'Copier la commande CURL', rotateAria: 'Actualiser la clé API', apiManualButton: 'Guide API', apiManualAutomationId: 'ID de l’automatisation actuelle', apiManualAutomationIdUnavailable: 'Généré après l’enregistrement', apiManualTitle: 'Guide de l’API externe', apiManualDescription: 'Utilisez l’identifiant de l’automatisation et la clé API pour appeler ces interfaces. Les domaines du certificat sont prédéfinis dans l’automatisation.', apiManualCertificateVersion: 'Les interfaces de lancement et de compatibilité exigent le certificateVersionId exact.', apiManualRunTitle: 'Démarrer l’automatisation', apiManualRunDescription: 'Envoyez une version de certificat pour lancer une exécution. L’exécution rejoint la file actuelle.', apiManualRunCurl: "curl -X POST 'https://gcac.example.com/api/v1/automation-external/AUTOMATION_ID/run' \\\n  -H 'X-Automation-API-Key: ak_xxx' \\\n  -H 'Content-Type: application/json' \\\n  -H Idempotency-Key:automation-run-$(date +%s) \\\n  -d '{'{'}\"certificateVersionId\":\"CERTIFICATE_VERSION_ID\"{'}'}'", apiManualPreviewTitle: 'Voir la compatibilité de l’application', apiManualPreviewDescription: 'Envoyez une version de certificat pour connaître les applications correspondantes, leur exécution possible et les exclusions.', apiManualPreviewCurl: "curl -X POST 'https://gcac.example.com/api/v1/automation-external/AUTOMATION_ID/preview' \\\n  -H 'X-Automation-API-Key: ak_xxx' \\\n  -H 'Content-Type: application/json' \\\n  -d '{'{'}\"certificateVersionId\":\"CERTIFICATE_VERSION_ID\"{'}'}'", apiManualVersionsTitle: 'Lister les versions de certificat disponibles', apiManualVersionsDescription: 'Retourne les versions sélectionnables pour les domaines de certificat prédéfinis dans cette automatisation.', apiManualVersionsCurl: "curl 'https://gcac.example.com/api/v1/automation-external/AUTOMATION_ID/certificate-versions' \\\n  -H 'X-Automation-API-Key: ak_xxx'" },
    ...
    manualRun: { title: 'Exécution manuelle', description: 'Sélectionnez une version de certificat avant d’exécuter.', versionLabel: 'Version du certificat', versionPlaceholder: 'Sélectionnez une version de certificat', help: 'L’exécution résout les actifs applicatifs liés à partir de la version sélectionnée.', empty: 'Aucune version de certificat n’est disponible pour une exécution manuelle.', stopOnError: 'Arrêter sur erreur', dryRun: 'Lancer une prévisualisation dry-run optionnelle', start: 'Démarrer', downgradeNotice: 'La validité du certificat cible est plus courte pour {count} actifs applicatifs. L’exécution continue après confirmation.', downgradeConfirmTitle: 'Confirmer la validité plus courte', downgradeConfirmDescription: 'Il s’agit d’une opération manuelle. Après confirmation, {count} actifs applicatifs seront mis à jour avec un certificat à validité plus courte.', downgradeConfirmAction: 'Confirmer et exécuter' },
    ...
    triggers: { onDemand: 'À la demande', onDemandDescription: 'Démarrée uniquement avec l’action d’exécution dans la plateforme. Aucune clé API externe n’est générée.', schedule: 'Planifiée' },
    ...
    eventSources: { acme_issue: 'Renouvellement automatique ACME', manual_import: 'Importation manuelle' },
    ...
    preview: { title: 'Impact sur les actifs', description: "Chaque ligne représente une application gérée ; la colonne centrale compare l'échéance actuelle à celle du certificat sélectionné, et la colonne de droite indique si la mise à jour peut être exécutée.", explanation: "L’analyse présente pour chaque application gérée l’échéance actuelle, l’échéance cible et le résultat d’exécution.", applicationAsset: 'Application gérée', certificate: 'Certificat', applicationAssetId: 'ID de l’application', bindingId: 'ID de liaison', missingCurrentExplanation: "« Aucune date → date cible » signifie que la plateforme n'a pas pu lire l'échéance du certificat actuellement lié ; la mise à jour reste donc bloquée jusqu'à la correction de la liaison ou des informations du certificat.", matched: '{count} correspondances', executable: '{count} exécutables', excluded: '{count} exclues', affected: '{count} actifs concernés', upgrade: '{count} validités prolongées', same: '{count} même échéance', skip: '{count} mises à jour ignorées', downgrade: '{count} à vérifier', version: 'Version {version}', versionUnknown: 'Version inconnue', ready: 'Prête', skipUpdate: 'Ignorer la mise à jour', expiryLabel: 'Expiration', impact: { upgrade: 'Validité prolongée', same: 'Même échéance', downgrade: 'Risque de validité réduite', missing_current: 'Certificat actuel absent', unknown: 'Impact inconnu' } },
    ...
    runDetail: { title: 'Détails de l’exécution automatisée', description: 'Version de configuration {version}', noFailure: 'Aucun échec', triggerContext: 'Trigger context', sourceType: 'Source type', certificateVersion: 'Exact certificate version', deliveryId: 'Delivery ID', excludedReasons: 'Excluded reasons' },
  ...
  executionDetail: {
    ...
    dryRun: {
      ...
      tlsGrantRequired: {
        ...
        detail: 'Les contrôles structurels et de sécurité sont terminés. Le dry-run ne délivre pas d’ExecutionGrant formel ; le contournement de la vérification TLS a donc été refusé. Lors de l’exécution réelle, l’hôte délivrera un ExecutionGrant temporaire.'
      },
      warning: {
  ...
  plugins: {
    ...
    filters: { searchLabel: 'Rechercher des plugins', searchPlaceholder: 'Rechercher par nom, tag, catégorie ou chemin', allSources: 'Toutes les sources', sourceAll: 'Tous', sourceBuiltin: 'Intégrés', sourceUser: 'Utilisateur', allStatuses: 'Tous les états', statusLabel: 'État du plugin' },
    ...
    capabilityKeys: {
      ...
      cloud_service_connection_test: 'Test de connexion au service cloud', cloud_service_discover: 'Découverte des services cloud', credential_health_check: 'Vérification de validité des identifiants'
    },
  ...
  gateways: {
    ...
    detail: {
      abilities: {
        ...
        relay: {
          description: 'Après une autorisation étroite, relaie uniquement les octets TCP vers la cible et le port approuvés.',
          title: 'Relais TCP direct'
        }
      },
  ...
  auditFormat: {
    ...
    taskTypes: { certificateDryRun: 'tâche de simulation de déploiement', certificateDeploy: 'tâche de déploiement de certificat', applicationCertificateDeploy: 'tâche de déploiement de certificat dédié d’application', certificateIssue: 'tâche d’émission de certificat', acmeCertificateIssue: 'tâche d’émission de certificat ACME', acmeRenewal: 'tâche de renouvellement de certificat ACME', agentInstall: 'tâche d’installation d’Agent', agentCapabilityRescan: 'tâche de nouvelle analyse des capacités Agent', pluginReferenceRefresh: 'tâche d’actualisation du catalogue de plugins', automationRun: 'tâche d’exécution automatisée', automationTriggerDelivery: 'tâche de remise du déclencheur automatisé', monitoring: 'tâche de surveillance des certificats', backgroundTask: 'tâche en arrière-plan' },
  ...
  settings: {
    ...
    deploymentTasks: {
      ...
      description: 'Contrôlez pour ce tenant si les déploiements de certificats exécutent une pré-vérification Dry-run.',
    ...
    version: {
      eyebrow: 'Gouvernance des versions',
      title: 'Informations de version',
      description: 'Choisissez un canal et obtenez les instructions de mise à jour manuelle sur l’hôte.',
      currentVersion: 'Version en cours',
      product: 'Produit',
      channelLabel: 'Canal de mise à jour',
      channelDescription: 'Cette page vérifie seulement les versions ; elle n’exécute ni Docker, ni mise à jour, ni retour arrière.',
      readonly: 'Ce compte a un accès en lecture seule.',
      channels: { stable: 'Stable', stableDescription: 'Pour la production ; privilégiez les versions validées.', dev: 'Développement', devDescription: 'Pour valider rapidement les builds de développement ; déconseillé en production.' },
      checkTitle: 'Rechercher des mises à jour',
      checkDescription: 'Vérifiez la version concrète du canal choisi et générez des commandes à copier sur l’hôte.',
      checkEmpty: 'Aucune vérification n’a encore été effectuée.',
      lastChecked: 'Vérifié à : {time}',
      targetVersion: 'Version cible',
      publishedAt: 'Publié le',
      releaseNotes: 'Notes de version',
      manualTitle: 'Instructions de mise à jour manuelle',
      manualDescription: 'Exécutez l’une des options sur l’hôte de déploiement. La page Web n’exécutera pas ces commandes.',
      installScriptCommand: 'install.sh',
      composeCommand: 'Docker Compose',
      relations: { current: 'Version du canal sélectionné déjà utilisée', upgrade: 'Mise à jour disponible', downgrade: 'La version cible est antérieure à la version actuelle', channel_switch: 'Le canal sélectionné a changé' },
      status: { current: 'Déjà à jour' },
      actions: { save: 'Enregistrer le canal', saving: 'Enregistrement…', check: 'Rechercher', checking: 'Vérification…' },
      messages: { channelSaved: 'Canal de mise à jour enregistré.' },
      errors: { emptySettings: 'Le backend n’a renvoyé aucun paramètre de canal.', emptyCheck: 'Le backend n’a renvoyé aucun résultat.', loadFailed: 'Impossible de charger le canal de mise à jour.', saveFailed: 'Impossible d’enregistrer le canal de mise à jour.', checkFailed: 'Échec de la vérification des mises à jour.' }
    },
    permissionPolicies: {
    ...
    roles: {
      ...
      actions: {
        ...
        deleting: 'Deleting...',
        clearSelection: 'Clear selection', revokePermission: 'Revoke permission', revoking: 'Revoking...'
      },
      columns: {
        roleId: 'Role ID',
        code: 'Code',
      ...
      presets: { label: 'Preset authorization template', custom: 'Custom business authorization', certificateViewer: 'Certificate viewer', certificateManager: 'Certificate manager', applicationViewer: 'Application viewer', applicationManager: 'Application manager' },
      effect: {
        allow: 'Allow',
        deny: 'Deny'
      },
      ...
      errors: {
        ...
        missingObjectSetId: 'The backend did not return an object scope ID', presetScopeMismatch: 'The preset does not match the selected object scope', revokePermissionFailed: 'Failed to revoke permission'
      },
      confirm: {
        deleteRole: 'Delete role "{name}"? This also removes its user assignments and object authorizations.', revokePermission: 'Revoke this business permission? Its compatibility object permission will also be removed.'
      },
      auditLogs: {
        auth: {
  ...
  assets: {
    ...
    card: {
      ...
      status: { valid: 'Valide', attention: 'Attention', unknown: 'Inconnu', executable: 'Exécutable', needsConfiguration: 'Configuration requise' },
    ...
    deployment: {
      ...
      description: 'Choisissez une version de certificat pour cet actif applicatif. Le système crée un instantané, lance la pré-vérification requise puis démarre le déploiement.',
      noCertificateAsset: 'Aucun actif de certificat déployable',
      dialogTitle: 'Déploiement du certificat',
      dialogDescription: 'Cette action concerne uniquement l’actif applicatif actuel. Le plan reste la limite de snapshot et d’exécution côté serveur.',
    ...
    certificateSupply: { title: 'Politique de fourniture de certificat', description: 'Configurez un certificat manuel ou dédié à l’application. Les versions sont conservées.', modeLabel: 'Mode de fourniture', manual: 'Sélectionner un certificat', dedicated: 'Utiliser un certificat dédié', certificateVersion: 'Version du certificat', selectCertificate: 'Sélectionner un certificat correspondant au domaine actuel', domainMatch: 'Seuls les certificats dont le CN ou le SAN couvre {domain} sont affichés.', provider: 'Provider d’émission', internalCa: 'Internal CA', acme: 'ACME', ca: 'Autorité de certification', selectCa: 'Sélectionner une CA', profile: 'Certificate Profile', selectProfile: 'Sélectionner une version de profil', acmeProvider: 'ACME Provider', acmeProfile: 'ACME Profile', selectAcmeProfile: 'Sélectionner un profil ACME', selectProvider: 'Sélectionner un Provider', dnsProvider: 'DNS Provider', selectDnsProvider: 'Sélectionner un DNS Provider', secretRef: 'SecretRef d’identifiant', secretRefPlaceholder: 'secret://tenant/path', custodyMode: 'Détention de clé', artifactMode: 'Mode d’artefact', canSave: 'Enregistrement possible', canIssue: 'Émission possible', canDeploy: 'Déploiement possible', lifecycle: 'État du cycle de vie', errors: { loadFailed: 'Impossible de charger la politique de fourniture.', previewFailed: 'Impossible de prévisualiser la politique de fourniture.' } },
    common: {
      required: 'Required',
      optional: 'Optional'
    }
  ...
  workflows: {
    ...
    canvasModel: {
      nodeTypes: {
        ...
        browser: {
          displayName: 'Étape navigateur',
          description: 'Naviguer, extraire ou vérifier des informations dans une session navigateur connectée.'
        },
        ssh: {
      ...
      fields: {
        ...
        browserUrl: 'URL de la page',
        browserExtractions: 'JSON des extractions',
        browserVerification: 'JSON de vérification'
      },
      options: {
  ...
  applicationOnboarding: {
    ...
    platforms: { customManual: 'Creation manuelle personnalisee', manualHint: 'Utiliser le parcours manuel', pluginHint: 'Parcours fourni par le module de plateforme', capabilityVersion: 'Version de capacite', compatibility: 'Versions compatibles', requiredInformation: 'Informations requises', inReview: 'Validation des capacites en cours', searchLabel: 'Rechercher des plateformes', searchPlaceholder: 'Rechercher par nom d’application, version de plateforme ou informations d’intégration', pluginCenterPrompt: 'Vous ne trouvez pas l’application souhaitée ?', pluginCenterAction: 'Voir le centre des plugins' },
