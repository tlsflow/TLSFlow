...
export default {
  ...
  internalCa: {
    ...
    adcsAgent: { title: 'Microsoft AD CS connections', description: 'Each AD CS CA uses an independent provider and agent. Add multiple domains, root CAs, or issuing CAs.', providerCount: '{count} Microsoft AD CS providers configured', addProvider: 'Add AD CS', wizardTitle: 'Add Microsoft AD CS', wizardDescription: 'Create an independent provider and a one-time installation command dedicated to that AD CS server.', multiProviderTitle: 'Multiple AD CAs are supported', multiProviderDescription: 'Each wizard run creates a new provider and never overwrites an existing AD CS connection.', providerName: 'AD CS provider name', createCommand: 'Create and generate command', commandReadyTitle: 'Installation command ready', commandReadyDescription: '{name} was created. Run this command in an elevated PowerShell session on the matching AD CS server.', expiresAt: 'Command expires at {time}', deleteProvider: 'Delete provider', deletingProvider: 'Deleting…', deleteConfirmText: 'DELETE', deleteProviderRisk: 'Deletes this provider and its unfinished enrollment tokens, agent nodes, tasks, and capability records. Providers bound to a certificate authority cannot be deleted.', deleteProviderBlocked: 'This provider is used by a certificate authority. Migrate or remove that authority first.', defaultProviderName: 'Microsoft AD CS Agent', defaultProviderNameIndexed: 'Microsoft AD CS {index}' },
