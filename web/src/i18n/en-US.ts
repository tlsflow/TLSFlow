...
export default {
  ...
  tasks: {
    ...
    typeLabels: {
      ACME_CERTIFICATE_ISSUE: 'ACME issue',
      ACME_CERTIFICATE_RENEWAL: 'ACME renewal',
      ...
      AGENT_CAPABILITY_RESCAN: 'Agent capability rescan',
  ...
  automations: {
    ...
    manualRun: { title: 'Manual run', description: 'Select a certificate version before running.', versionLabel: 'Certificate version', versionPlaceholder: 'Select a certificate version', help: 'The run will resolve related application assets from the selected version.', empty: 'No certificate versions are available for manual run.', stopOnError: 'Stop on error', dryRun: 'Run dry-run first', start: 'Start run' },
  ...
  internalCa: {
    ...
    adcsAgent: { title: 'Connect an existing Microsoft AD CS', description: 'Deploy the GCAC adapter on a Windows Server where AD CS is already installed and configured.', providerCount: '{count} Microsoft AD CS connections configured', addProvider: 'Connect existing AD CS', updateAgent: 'Update Agent', updateTitle: 'Update Microsoft AD CS Agent', updateDescription: 'Generate a one-time update command for the existing Provider without creating a new Provider or changing the managed CA.', updateCommand: 'Generate update command', wizardTitle: 'Connect an existing Microsoft AD CS', wizardDescription: 'Create an independent connection for a server already running an AD CS certification authority and generate its adapter installation command.', requirementsTitle: 'The target server must already have', requirementInstalled: 'The Active Directory Certificate Services Certification Authority role service installed.', requirementConfigured: 'An initialized Enterprise CA or Standalone CA with an active CA Config.', requirementService: 'The CertSvc service running.', requirementPermission: 'An installation and service account with the required template enrollment, CA query, and administration permissions.', connectionName: 'AD CS connection name', coexistenceTitle: 'Coexists with other GCAC agents', coexistenceDescription: 'The AD CS Agent has a dedicated service name and directories, receives tasks and returns results over an outbound persistent connection, and does not listen on a local port. It can coexist with the device-management Agent or Gateway Agent on the same Windows Server.', createCommand: 'Create connection and generate Agent command', commandReadyTitle: 'Adapter installation command ready', commandReadyDescription: '{name} was created. Run this command in an elevated PowerShell session on the server hosting the existing AD CS CA.', expiresAt: 'Command expires at {time}', deleteProvider: 'Delete provider', deletingProvider: 'Deleting…', deleteConfirmText: 'DELETE', deleteProviderRisk: 'Deletes this provider and its unfinished enrollment tokens, agent nodes, tasks, and capability records. Providers bound to a certificate authority cannot be deleted.', deleteProviderBlocked: 'This provider is used by a certificate authority. Migrate or remove that authority first.', defaultProviderName: 'Microsoft AD CS Agent', defaultProviderNameIndexed: 'Microsoft AD CS connection {index}' },
