...
export default {
  ...
  tasks: {
    ...
    values: { system: 'System', externalApi: 'External API', empty: 'No records', none: 'None' },
    agentUpdate: {
    ...
    dedicated: {
      description: { supply: 'The system is coordinating dedicated certificate issuance, deployment, and result confirmation.', issue: 'The dedicated certificate is being issued for this application.', deploy: 'The dedicated certificate is being deployed to this application.' },
    ...
    typeLabels: {
      ...
      APPLICATION_CERTIFICATE_SUPPLY: 'Dedicated certificate processing',
  ...
  automations: {
    ...
    triggers: { onDemand: 'On demand', onDemandDescription: 'Started only by the in-platform run action. No external API key is generated.', schedule: 'Scheduled' },
    ...
    eventSources: { acme_issue: 'ACME automatic renewal', manual_import: 'Manual import' },
    ...
    preview: { title: 'Application impact preview', description: 'Each row is one managed application; the middle column shows current certificate expiry → selected certificate expiry, and the right column shows whether the update can run.', explanation: 'Impact analysis shows each managed application’s current certificate expiry, target certificate expiry, and execution result.', applicationAsset: 'Application asset', certificate: 'Certificate', applicationAssetId: 'Application asset ID', bindingId: 'Binding ID', missingCurrentExplanation: '“No current date → target date” means the platform could not read the expiry of the application’s current certificate, so this target will not be updated until its binding or current certificate information is completed.', matched: '{count} matched', executable: '{count} executable', excluded: '{count} excluded', affected: '{count} affected', upgrade: '{count} longer validity', same: '{count} same expiry', skip: '{count} skipped', downgrade: '{count} need attention', version: 'Version {version}', versionUnknown: 'Version unknown', ready: 'Ready', skipUpdate: 'Skip update', expiryLabel: 'Expiry', impact: { upgrade: 'Longer validity', same: 'Same expiry', downgrade: 'Shorter validity risk', missing_current: 'Current certificate missing', unknown: 'Unknown impact' } },
  ...
  dashboard: {
    ...
    quickStart: {
      ...
      description: 'Manage certificates and deploy them to applications.',
      addCertificate: 'Add certificate',
      deployExistingApplication: 'Deploy certificate',
    ...
    trends: {
      ...
      noDelta: '--',
      auditSuccess: { title: 'Audit success rate', suffix: 'successful' },
      managedObjects: { title: 'System health', suffix: 'healthy objects' },
      certificateAttention: { title: 'Certificates to review', suffix: '' }
    },
      statusPanel: {
    ...
    loading: {
      description: 'Loading overview data.',
      title: 'Loading'
    },
    metrics: {
      attention: 'Warning',
      ...
      activeGateways: {
        ...
        description: 'Gateways currently online in isolated zones.'
      },
      applications: {
        title: 'Applications',
        description: 'Managed application assets.'
      },
      ...
      validCertificates: {
        title: 'Valid certificate versions',
        description: 'Active, unexpired certificate versions.'
      }
    },
    health: {
      title: 'System health',
    ...
    quickActions: {
      ...
      gateways: {
        title: 'Gateways',
        description: 'Manage gateways for isolated zones.'
      }
    },
    statusBlock: {
      tooltip: {
    ...
    statusGroups: {
      ...
      summary: {
        allNormal: 'Healthy',
        needsAttention: '{count} requiring attention'
      }
    },
    table: {
      bindings: 'Bindings',
  ...
  settings: {
    ...
    version: {
      ...
      description: 'Choose a release channel and get host-side manual update instructions.',
      currentVersion: 'Running version',
      product: 'Product',
      channelLabel: 'Update channel',
      channelDescription: 'The web page only checks versions; it never runs Docker, upgrades, or rollbacks.',
      readonly: 'This account has read-only access.',
      channels: { stable: 'Stable', stableDescription: 'For production; prefer versions that have completed validation.', dev: 'Development', devDescription: 'For early validation of development builds; not recommended for production.' },
      checkTitle: 'Check for updates',
      checkDescription: 'Check the concrete version for the selected channel and generate copyable host commands.',
      checkEmpty: 'No update check has been run yet.',
      lastChecked: 'Checked at: {time}',
      targetVersion: 'Target version',
      publishedAt: 'Published at',
      releaseNotes: 'Release notes',
      manualTitle: 'Manual update instructions',
      manualDescription: 'Run either option on the deployment host. The web page will not execute these commands for you.',
      installScriptCommand: 'install.sh',
      composeCommand: 'Docker Compose',
      relations: { current: 'Already on the selected channel version', upgrade: 'Update available', downgrade: 'Target version is older than the current version', channel_switch: 'Selected channel changed' },
      status: { current: 'Already up to date' },
      actions: { save: 'Save channel', saving: 'Saving…', check: 'Check for updates', checking: 'Checking…' },
      messages: { channelSaved: 'Update channel saved.' },
      errors: { emptySettings: 'The backend returned no update channel settings.', emptyCheck: 'The backend returned no update check result.', loadFailed: 'Failed to load the update channel.', saveFailed: 'Failed to save the update channel.', checkFailed: 'Update check failed.' }
    },
    permissionPolicies: {
  ...
  assets: {
    ...
    deployment: {
      ...
      errors: { missingApplicationAssetId: 'Managed application ID is required to create a certificate deployment.', missingCertificateVersion: 'The current certificate supply policy has no deployable certificate version.', loadOptionsFailed: 'Failed to load deployable certificate versions.', createPlanMissingId: 'The deployment snapshot was created without a plan ID.', createTaskMissingId: 'The dedicated certificate deployment task was created without a task ID.', deployFailed: 'Certificate deployment failed.', preflightFailed: 'The certificate deployment preflight did not pass.', preflightTimeout: 'The certificate deployment preflight timed out.', loadRecordsFailed: 'Failed to load managed application deployment records.' },
      dedicated: { kicker: 'Dedicated certificate', title: 'Application dedicated certificate', providerTypes: { acme: 'ACME', internalCa: 'Managed CA' }, fields: { providerType: 'Provision method', ca: 'Selected CA', caStatus: 'CA status', custodyMode: 'Private key management', certificate: 'Certificate status', issuedAt: 'Requested at', expiresAt: 'Expires at', remainingDays: 'Days remaining' }, status: { available: 'Available', unavailable: 'Unavailable', unknown: 'Unknown' }, custody: { agentLocal: 'Managed by Agent', managedSecret: 'Managed by platform' }, certificate: { exists: 'Available', missing: 'Not issued', failed: 'Issuance failed' }, remainingDays: '{days} days remaining', reapply: 'Request a new certificate', reapplyHint: 'Request a new dedicated certificate. Used for certificate private-key rotation.', deployCurrentHint: 'Deploy the current dedicated domain certificate.', issuancePending: 'The dedicated certificate request was submitted. Deploy it after issuance completes.', issuanceFailed: 'Dedicated certificate issuance failed. Review the failed task details and retry.', issuanceFailedWithCode: 'Dedicated certificate issuance failed: {message} ({code})' }
    },
    compatibilityModes: {
    ...
    certificateSupply: { title: 'Certificate configuration', description: 'Configure a manual or dedicated application certificate. Previous configurations are retained.', userModeDescription: 'GCAC automatically requests and deploys an HTTPS certificate using the tenant default policy. No Provider or Profile selection is required.', userStatus: { applying: 'Requesting', deploying: 'Deploying', effective: 'Active', attention: 'Needs attention' }, userStatusDescriptions: { applying: 'The certificate is being requested or renewed.', deploying: 'The certificate was issued and is being deployed to the application.', effective: 'The certificate is deployed and verified.', attention: 'No default policy matched, or an administrator needs to intervene.' }, modeLabel: 'Certificate configuration mode', manual: 'Select a certificate', dedicated: 'Use a dedicated certificate', certificate: 'Certificate', certificateVersion: 'Certificate version', selectCertificate: 'Select a certificate domain matching the current domain', domainMatch: 'Only certificate domains whose CN or SAN covers {domain} are shown.', provider: 'Issuing provider', internalCa: 'Managed CA', acme: 'ACME', ca: 'Certificate authority', selectCa: 'Select a CA', profile: 'Certificate Profile', selectProfile: 'Select a profile version', acmeProvider: 'ACME Provider', acmeProfile: 'ACME Profile', selectAcmeProfile: 'Select an ACME profile', selectProvider: 'Select a provider', dnsProvider: 'DNS Provider', selectDnsProvider: 'Select a DNS provider', secretRef: 'Credential SecretRef', secretRefPlaceholder: 'secret://tenant/path', custodyMode: 'Key custody', artifactMode: 'Deployment artifact mode', canSave: 'Can save', canIssue: 'Can issue', canDeploy: 'Can deploy', lifecycle: 'Lifecycle status', errors: { loadFailed: 'Failed to load certificate configuration.', previewFailed: 'Failed to preview certificate configuration.' } },
    common: {
      required: 'Required',
      optional: 'Optional'
    }
  ...
  certificates: {
    ...
    list: {
      ...
      versions: {
        ...
        titleWithDomain: 'Certificate versions for {domain}',
        description: 'Review certificate versions for this domain, including validity, issuer, subject, and source.',
        loadFailed: 'Failed to load certificate versions',
        emptyForDomain: 'No certificate versions found',
        emptyForDomainDescription: 'Import a certificate to add a version for this domain.',
        empty: 'No certificate versions',
        toolbar: 'Versions',
        currentCount: '{count} versions'
      },
      actions: {
        toggleFilters: 'Filter',
        clear: 'Reset',
        deleteRisk: 'Delete permanently removes this certificate version. Deletion is blocked while it is referenced by a binding or deployment.'
      },
      errors: {
  ...
  internalCa: {
    ...
    trustDomains: { recordsTitle: 'Trust domain records', columns: { name: 'Name', purpose: 'Purpose', isolationLevel: 'Isolation level', status: 'Status', default: 'Default', createdAt: 'Created at', actions: 'Actions' }, empty: 'No CA trust domains yet.', modalTitle: 'Add CA trust domain', modalDescription: 'Enter the trust domain basics. The unique code is generated automatically.', editModalTitle: 'Edit CA trust domain', editModalDescription: 'Update the trust domain basics and its default status.', generatedCodeHint: 'The unique code is generated automatically and does not need to be entered manually.', notDefault: 'Not default' },
    authorityEdit: { modalTitle: 'Edit CA', modalDescription: 'Update the CA management settings. Certificate subject, private key, hierarchy, and provider remain unchanged.', selectProfile: 'Select an existing certificate profile', noProfiles: 'No active Internal CA profiles are compatible with this CA provider.' },
    operations: { title: 'Lifecycle operations', policiesTitle: 'Certificate policies', bindingsTitle: 'Provider action bindings', rotationsTitle: 'Key rotations', crlTitle: 'CRL publications', policyVersion: 'Current version: {version}', policyApproval: 'Issuance approval: {required}', emptyPolicies: 'No certificate policy records.', emptyBindings: 'No external CA action bindings.', emptyRotations: 'No certificate rotation records.', emptyCrl: 'No CRL publication records.', plan: 'Deployment plan: {id}', targetVersion: 'Target certificate version: {id}', tlsStatus: 'TLS verification: {status}', tlsPending: 'Pending', rollbackRequired: 'Rollback or manual action required', crlNumber: 'CRL number: {number}', publishCrl: 'Republish CRL', publishCrlFor: 'Publish CRL for {name}', crlPublished: 'CRL published.' },
    requests: { recordsTitle: 'Certificate request records', columns: { commonName: 'Common name', applicationAssetId: 'Managed application ID', keyReference: 'Key custody', certificate: 'Certificate and deployment', updatedAt: 'Updated at', actions: 'Actions' }, empty: 'No certificate requests.', modalTitle: 'New certificate request', modalDescription: 'Fill in the certificate request details. It will enter the approval and issuance flow after submission.', policyVersion: 'Policy: {id}', certificateVersion: 'Certificate version: {id}', deployment: 'Deployment: {status}', deploymentPlan: 'Plan: {id}', deploymentActive: 'Active on target', deploymentBlocked: 'Deployment blocked', deploymentPending: 'Not deployed', localAgentHint: 'The local agent creates the private key and CSR on the target. Only public CSR evidence returns to GCAC.', agentId: 'Agent ID', targetId: 'Target ID', keyPath: 'Private key path', certificatePath: 'Certificate output path', configPath: 'Local Tomcat config path', alias: 'KeyStore alias', format: 'Certificate format', storageMode: 'Key storage', storageFile: 'PEM file', storageCng: 'Windows CNG/KSP', localAgentRequestIdMissing: 'The certificate request ID was not returned; the local CSR task was not sent.' },
    profiles: { recordsTitle: 'Certificate profile records', columns: { providerType: 'Provider type', securityDomain: 'Security domain', versionCount: 'Version count', actions: 'Actions' }, empty: 'No certificate profiles.', modalTitle: 'New certificate profile', modalDescription: 'Create reusable HTTPS certificate rules, then associate them with a certificate authority.', providerTypes: { acme: 'Public ACME', internalCa: 'Enterprise Internal CA' }, anyProvider: 'Any active provider', anyAuthority: 'Any active CA', anyAcmeProviderProfile: 'Use provider default', noDnsProvider: 'No DNS provider', domainPatterns: 'Domain match patterns', domainPatternsPlaceholder: 'example.com, *.example.com', applicableDomains: 'Applicable domains', applicableDomainsPlaceholder: '*.example.com, *.example.org', applicableDomainsHint: 'Leave empty for no restriction. Entered domains are used for both Profile matching and DNS suffix validation.', targetCapabilities: 'Required target capabilities', targetCapabilitiesPlaceholder: 'certificate.deploy, key.generate_csr', securityDomainPlaceholder: '* for all security domains', priority: 'Administrative priority', defaultProfile: 'Default automatic profile', credentialRefPlaceholder: 'secret://tenant/path' },
    ...
    fields: { name: 'Name', code: 'Code', purpose: 'Purpose', isolationLevel: 'Isolation level', defaultTrustDomain: 'Set as default trust domain', trustDomain: 'CA trust domain', parentAuthority: 'Parent root CA', authorityType: 'Authority type', deploymentMode: 'Deployment mode', platform: 'Runtime platform', backendName: 'Issuing backend name', availabilityMode: 'Availability mode', endpoint: 'Service endpoint', authMode: 'Authentication mode', profile: 'Issuance profile', certificateProfiles: 'Certificate profiles', template: 'Certificate template', crlUrl: 'CRL URL', ocspUrl: 'OCSP URL', issuingBackend: 'Issuing backend', entryMode: 'Creation mode', commonName: 'Common name', certificateSubjectCommonName: 'Certificate subject common name', securityDomain: 'Security domain', topology: 'CA topology', dnsSuffixes: 'Allowed DNS suffixes', validityDays: 'Maximum validity days', renewalDays: 'Renewal window days', requireApproval: 'Require approval before issuance', applicationAssetId: 'Managed application ID', authority: 'Certificate authority', providerType: 'Provider type', profileVersionId: 'Profile version ID', acmeProviderProfile: 'ACME provider profile', dnsProvider: 'DNS provider', credentialRef: 'Credential SecretRef', sans: 'SAN list', custodyMode: 'Key custody mode', certificateVersionId: 'Certificate version ID', reason: 'Revocation reason', targetIds: 'Target ID list' },
    actions: { refresh: 'Refresh', addTrustDomain: 'Add trust domain', editTrustDomain: 'Edit trust domain', updateTrustDomain: 'Save trust domain', deleteTrustDomain: 'Delete trust domain', addAuthority: 'Add CA', editAuthority: 'Edit CA', updateAuthority: 'Save CA', deleteAuthority: 'Delete CA', associateProfile: 'Associate', removeProfileAssociation: 'Remove association', addIntermediate: 'Add intermediate CA', previous: 'Previous', next: 'Next', createTrustDomain: 'Create trust domain', previewRisk: 'Preview risk', createAuthority: 'Create CA', createProfile: 'Create profile', deleteProfile: 'Delete profile', createRequest: 'Submit request', approve: 'Approve', retry: 'Retry', queryResult: 'Query result', scanRenewals: 'Scan renewals', createRevocation: 'Create revocation', createTrust: 'Create trust-store distribution', previewRemediation: 'Preview remediation' },
    placeholders: { dnsSuffixes: 'example.com, office.example.com', sans: 'oa.example.com, 10.0.0.10' },
    messages: { loadFailed: 'Failed to load internal CA data.', actionFailed: 'The operation failed. Check input, permissions, and approval status.', confirmAuthorityDelete: 'Retire CA "{name}"? Historical certificates and audit records will be retained.', authorityDeleted: 'CA retired.', authorityUpdated: 'CA settings updated.', confirmTrustDomainDelete: 'Delete trust domain "{name}"? It will no longer be available for new issuance, while historical certificates and audit records are retained.', trustDomainDeleted: 'CA trust domain deleted.', trustDomainUpdated: 'CA trust domain updated.', confirmProfileDelete: 'Delete certificate profile "{name}"? It will no longer participate in automatic resolution, while historical versions and requests are retained.', profileDeleted: 'Certificate profile deleted.', noIntermediate: 'No intermediate CA configured.', noRootAuthority: 'No root CA configured', noRootAuthorityDescription: 'Add a root CA to establish the first independent trust hierarchy.', trustDomainCreated: 'CA trust domain created.', authorityCreated: 'CA created.', profileCreated: 'Certificate profile created.', requestCreated: 'Certificate request submitted.', requestApproved: 'Certificate request approved.', requestRetried: 'Certificate issuance retried.', requestQueried: 'Remote issuance result refreshed.', renewalScanned: 'Renewal scan completed.', revocationCreated: 'Revocation task created and awaiting approval.', revocationApproved: 'Certificate revocation approved.', trustCreated: 'Trust-store distribution created and awaiting approval.', trustApproved: 'Trust-store distribution approved.' },
    ...
    backendSummary: { createAndIssue: 'Can create and issue certificates', requestPublicCertificates: 'Can request public certificates', external: 'Requires an external integration', localVerified: 'Local verification passed', remoteVerified: 'Connection verification passed', unverified: 'Not verified', agentVersion: 'Agent version', agentVersionSource: 'Version source', versionFromHeartbeat: 'Latest heartbeat (running binary)', versionFromRegistration: 'Registration record (no heartbeat yet)', registeredVersion: 'Registered version', agentKey: 'Agent key', agentStatus: 'Agent status', heartbeatAt: 'Last heartbeat', observationAt: 'Last observation', storedRecords: 'Stored records', observationStats: 'Scan {scanned} · send {submitted} · insert {inserted}', refresh: 'Scan and refresh', refreshing: 'Scanning…', noAgent: 'No AD CS Agent is associated with this CA.', statusOnline: 'Working', statusOffline: 'Offline', statusUnknown: 'Unknown', observationNever: 'No observation received yet', refreshSucceeded: 'AD CS scan completed: {count} new records stored.', refreshFailed: 'AD CS scan failed.' },
    ...
    protectionLevels: { softwareControlled: 'Software controlled', osProtected: 'OS protected', hardwareBacked: 'Hardware backed' },
  ...
  applicationOnboarding: {
    eyebrow: 'Application onboarding', title: 'Add managed application', description: 'Choose a platform and complete device, site, and certificate setup.', helpAria: 'Application onboarding help', stepsAria: 'Application onboarding steps',
    steps: { platform: 'Platform', device: 'Asset', target: 'Site', certificate: 'Certificate', complete: 'Complete' },
    platforms: { customManual: 'Custom manual setup', manualHint: 'Use the traditional manual flow', pluginHint: 'Fixed flow supplied by the platform plugin', capabilityVersion: 'Plugin', compatibility: 'Compatibility', requiredInformation: 'Prerequisites', inReview: 'Capability validation is in progress', searchLabel: 'Search platforms', searchPlaceholder: 'Search by application name, platform version, or onboarding information', pluginCenterPrompt: "Can't find the application you need?", pluginCenterAction: 'Browse the plugin center' },
    ...
    resource: { title: 'Choose platform resource', refresh: 'Refresh resources', loading: 'Loading available resources.', empty: 'No platform resources are available.' },
    target: { title: 'Choose site', siteName: 'Site name', selectedSite: 'Selected site', accessDomain: 'Access domain', verifyUrl: 'Verification URL', accessDomainPlaceholder: 'e.g. ikuai.jacksonz.cn', verifyUrlPlaceholder: 'e.g. https://ikuai.jacksonz.cn:443', domainHint: 'The management endpoint may be an IP, but the access domain and verification URL must use the same DNS name.', invalidConfiguration: 'Enter a valid DNS access domain and a verification URL on the same host.', listenAddress: 'Listen address', listenPort: 'Listen port', protocol: 'Protocol', selectable: 'Selectable managed target', notSelectable: 'Not selectable', unavailableReason: 'Why it cannot be selected', missingValue: 'Not provided', reasons: { managedTargetInactive: 'This managed target is inactive.', workflowCapabilityMissing: 'This target does not provide the workflow capability required by this platform.', targetEndpointMissing: 'This target is missing a complete listen address, port, or protocol.', unknown: 'This target does not currently meet the selection requirements.' } }, certificate: { title: 'Configure certificate', modeLabel: 'Certificate mode', manual: 'Select certificate manually', manualDescription: 'Choose an issued version from the certificate inventory.', dedicated: 'Use dedicated certificate', dedicatedDescription: 'Request and renew a certificate for this application.', testPhase: 'Testing', provider: 'Issuance method', internalCa: 'Internal CA', acme: 'ACME', dedicatedHint: 'Dedicated certificate settings can be completed in application settings.', asset: 'Certificate inventory item', version: 'Certificate version', latest: 'Always use latest', requiredFormat: 'This platform requires a certificate in {formats} format', noMatchingAssets: 'No certificate inventory item matches “{domain}”.' },
    automation: { title: 'Certificate update automation', triggerLabel: 'Trigger', none: 'Do not configure now', noneDescription: 'Create the application first and configure this later in application settings.', certificateVersionCreated: 'Update when a new certificate version is created', certificateVersionCreatedDescription: 'Create an update task for this application whenever the certificate inventory gets a new version.', once: 'Run once at a specified time', onceDescription: 'Run one time, useful for a launch or maintenance window.', schedule: 'Update on a fixed schedule', scheduleDescription: 'Check and update every day, week, or month at a chosen local time.', runAt: 'Run at', onceHint: 'The time uses your browser’s local time.', frequency: 'Frequency', daily: 'Every day', weekly: 'Every week', monthly: 'Every month', scheduleDay: 'Day', scheduleTime: 'Time', scheduleHint: 'The time uses your browser’s local time.', monday: 'Monday', tuesday: 'Tuesday', wednesday: 'Wednesday', thursday: 'Thursday', friday: 'Friday', saturday: 'Saturday', sunday: 'Sunday', cron: 'Cron expression', defaultName: 'Application certificate auto-update', defaultDescription: 'Created by application onboarding.' },
    complete: { title: 'Onboarding complete', description: 'Review the first four steps, then confirm to create the application and related tasks.', progressAria: 'Creation progress', viewSummary: 'View configuration summary', stages: { application: 'Create application', certificate: 'Link certificate', automation: 'Create automation' }, status: { pending: 'Pending', running: 'In progress', success: 'Completed', failed: 'Failed' } },
    actions: { customManual: 'Manual setup', openWizard: 'Use onboarding wizard', previous: 'Previous', continue: 'Continue', refresh: 'Refresh sites', review: 'Review summary', complete: 'Confirm and create', close: 'Close', cancel: 'Cancel wizard' },
