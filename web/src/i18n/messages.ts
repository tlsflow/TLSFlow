...
const zhCN = {
  ...
  assets: {
    ...
    fields: {
      assetId: '应用资产 ID', domain: '访问域名', addressType: '地址类型', port: '端口', protocol: '协议', verifyUrl: '验证 URL', platform: '平台', frameworkType: '框架类型', selectWorkflow: '选择工作流', workflowVersionSelection: '版本更新方式',
    ...
    workflowTarget: { title: '应用资产属性', description: '用于应用资产展示、部署后探测和 DSL 目标变量同步。', dslSyncHint: '已同步到 DSL 目标变量', advancedTitle: '高级设置', advancedDescription: '仅在需要覆盖默认监听、请求域名或 TLS 证书域名时修改。', expandAdvanced: '展开高级设置', collapseAdvanced: '收起高级设置', bindingInformationLabel: '服务监听规则', bindingInformationHelp: '用于描述服务监听的地址、端口和域名组合。', hostHeaderLabel: '访问请求域名', hostHeaderHelp: '仅在目标服务要求指定 HTTP Host 请求头时修改。', sniNameLabel: 'TLS 证书域名', sniNameHelp: '仅在 TLS 握手域名与访问域名不一致时修改。' },
    workflowVersionSelection: { pinned: '固定当前版本', latestPublished: '始终使用最新已发布版本' },
    workflowVariables: {
...
const enUS = {
  ...
  assets: {
    ...
    fields: {
      assetId: 'Application asset ID', domain: 'Domain', addressType: 'Address type', port: 'Port', protocol: 'Protocol', verifyUrl: 'Verify URL', platform: 'Platform', frameworkType: 'Framework type', selectWorkflow: 'Select workflow', workflowVersionSelection: 'Version update policy',
    ...
    workflowTarget: { title: 'Application asset properties', description: 'Used for application asset display, post-deploy probing, and DSL target variable synchronization.', dslSyncHint: 'Synced to DSL target variables', advancedTitle: 'Advanced settings', advancedDescription: 'Change only when overriding the default listener, request host, or TLS certificate name.', expandAdvanced: 'Expand advanced settings', collapseAdvanced: 'Collapse advanced settings', bindingInformationLabel: 'Service listener rule', bindingInformationHelp: 'Describes the address, port, and host combination used by the service listener.', hostHeaderLabel: 'Request host name', hostHeaderHelp: 'Change only when the target service requires a specific HTTP Host header.', sniNameLabel: 'TLS certificate name', sniNameHelp: 'Change only when the TLS handshake name differs from the access host.' },
    workflowVersionSelection: { pinned: 'Pin current version', latestPublished: 'Always use latest published version' },
    workflowVariables: {
