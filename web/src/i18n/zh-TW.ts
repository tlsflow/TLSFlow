...
export default {
  ...
  internalCa: {
    ...
    tabs: { authorities: '憑證機構', profiles: '憑證 Profile', requests: '憑證申請', operations: '生命週期維運', risks: '重用風險' },
    ...
    sections: { provider: 'CA Provider', authorityWizard: 'CA 建立精靈', riskSummary: '安全決策摘要', profile: '建立憑證 Profile', request: '建立應用憑證申請', revocation: '建立撤銷工作', trust: '建立信任散發工作', remediation: '改善預覽' },
    fields: { name: '名稱', providerType: 'Provider 類型', deploymentMode: '部署模式', platform: '執行平台', provider: 'CA Provider', commonName: 'Common Name', securityDomain: '安全域', topology: 'CA 拓撲', dnsSuffixes: '允許的 DNS 後綴', validityDays: '最大有效期（天）', renewalDays: '提前續期（天）', requireApproval: '簽發前需要審批', applicationAssetId: '應用資產 ID', authority: '憑證機構', profileVersionId: 'Profile 版本 ID', sans: 'SAN 清單', custodyMode: '金鑰託管模式', certificateVersionId: '憑證版本 ID', reason: '撤銷原因', targetIds: '目標 ID 清單' },
    actions: { refresh: '重新整理', createProvider: '建立 Provider', previewRisk: '預覽風險', createAuthority: '建立 CA', createProfile: '建立 Profile', createRequest: '提交申請', approve: '審批通過', retry: '重試', queryResult: '查詢結果', scanRenewals: '掃描到期續期', createRevocation: '建立撤銷工作', createTrust: '建立信任散發', previewRemediation: '預覽改善' },
    placeholders: { dnsSuffixes: 'example.com, office.example.com', sans: 'oa.example.com, 10.0.0.10' },
    messages: { loadFailed: '內部 CA 資料載入失敗。', actionFailed: '操作失敗，請檢查輸入、權限和審批狀態。', providerCreated: 'CA Provider 已建立。', authorityCreated: '憑證機構已建立。', profileCreated: '憑證 Profile 已建立。', requestCreated: '憑證申請已提交。', requestApproved: '憑證申請已審批。', requestRetried: '憑證簽發已重試。', requestQueried: '遠端簽發結果已重新整理。', renewalScanned: '續期掃描已完成。', revocationCreated: '撤銷工作已建立並等待審批。', revocationApproved: '憑證撤銷已審批。', trustCreated: '信任散發工作已建立並等待審批。', trustApproved: '信任散發已審批。' },
    metrics: { nodes: 'CA Node', renewals: '續期工作', revocations: '撤銷工作', trust: '信任散發', totalRisks: '風險總數', critical: '嚴重風險', affectedAssets: '受影響應用資產' },
    labels: { versionCount: '{count} 個版本', assetCount: '{count} 個應用資產', requestCount: '將建立 {count} 個獨立憑證申請' },
