...
export default {
  ...
  automations: {
    ...
    fields: { name: "名稱", description: "說明", trigger: "觸發方式", cron: "Cron 運算式", timeZone: "時區", expiresWithinDays: "到期天數範圍", environments: "目標環境（逗號分隔）", maxTargets: "單次最大目標數", concurrency: "並行數", failureCount: "失敗數量閾值", requireDryRun: "執行前必須完成 Dry Run", requireApproval: "執行前必須審批", startedAt: "開始時間", finishedAt: "結束時間", failureStage: "失敗階段", parentRun: "父執行" },
