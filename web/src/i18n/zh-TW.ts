...
export default {
  ...
  internalCa: {
    ...
    wizard: { title: '新增憑證機構', description: '使用內建簽發方式，再逐步設定 CA 參數與安全邊界。', stepsAria: 'CA 建立步驟', entryStep: '選擇方式', backendStep: '設定後端', parentStep: '選擇父 CA', authorityStep: '設定 CA', reviewStep: '確認建立', completed: '已完成', inProgress: '進行中', pending: '待填寫', entryEyebrow: '第一步', entryTitle: '這套 CA 由誰負責簽發？', entryDescription: '目前版本僅支援由 GCAC 服務內建執行面完成簽發。', recommended: '建議起步', builtinTitle: '直接建立 CA', builtinDescription: '由目前 GCAC 服務內建的通用憑證簽發執行面完成。', builtinFeature1: '不需部署額外節點', builtinFeature2: '適合開發與中小型內部環境', backendEyebrow: '簽發後端', builtinBackendTitle: '使用 GCAC 內建簽發後端', builtinBackendDescription: '系統會自動建立或重用租戶內建執行後端，使用者只需設定 CA。', builtinAutomaticTitle: '不需要另外建立執行後端', builtinAutomaticDescription: '建立 CA 時系統會自動確保內建簽發後端存在並完成綁定。', authorityEyebrow: '憑證機構', rootConfigurationTitle: '設定根 CA', rootConfigurationDescription: '定義新的根信任邊界、名稱、主體與中繼 CA 拓撲。', intermediateConfigurationTitle: '設定中繼 CA', intermediateConfigurationDescription: '先選擇父根 CA，再設定承擔日常簽發的中繼憑證機構。', advancedSubjectTitle: '進階憑證主體設定', commonNameHelp: '寫入 CA 憑證主體，用於憑證鏈識別，不是網域名稱。', builtinSecurityNote: '軟體私鑰由 GCAC SecretService 託管，不等同於不可匯出的 HSM 金鑰。', reviewEyebrow: '最後確認', reviewTitle: '檢查信任邊界與簽發方式', reviewDescription: '確認 CA 名稱、信任域、簽發後端與風險提示後再建立。', builtinProviderName: 'GCAC 內建簽發後端', rootTitle: '根 CA', rootDescription: '建立新的獨立根信任錨點，並可同時建立首個中繼 CA。', intermediateTitle: '中繼 CA', intermediateDescription: '掛載至現有根 CA 下承擔日常簽發，不建立新的根信任邊界。', noWarnings: '未發現額外的拓撲風險警告。' },
