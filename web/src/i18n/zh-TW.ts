...
export default {
  ...
  internalCa: {
    ...
    backendTypes: { builtin: '內建簽發後端', managedNode: '獨立 CA Node', acme: '公共 ACME 憑證機構', external: '外部簽發後端' },
    backendSummary: { createAndIssue: '可建立和簽發憑證', requestPublicCertificates: '可申請公共憑證', managedNode: '需先註冊 CA Node', external: '需接入外部執行器', localVerified: '本機驗證通過', remoteVerified: '連線驗證通過', unverified: '尚未驗證' },
  ...
  applicationOnboarding: {
    ...
    platforms: { customManual: '自訂手動建立', manualHint: '使用傳統手動流程', pluginHint: '由平台外掛提供固定流程', capabilityVersion: '接入能力版本', compatibility: '支援的平台版本', requiredInformation: '接入前需提供', inReview: '功能驗證中，暫時無法接入' },
    device: { title: '連接業務平台', existing: '使用現有裝置', new: '新增裝置', deviceId: '裝置 ID', selectPlaceholder: '請選擇裝置', noExisting: '沒有可用於此平台的健康裝置。', existingLoading: '正在載入相容裝置。', refreshExisting: '重新整理裝置', newDescription: '將開啟統一裝置接入向導，完成 Agent 註冊或裝置接入後返回本向導。', newHint: '返回後系統會重新載入此平台已發現的可用站點裝置。', newAction: '開啟裝置接入向導', username: '使用者名稱', password: '密碼', host: '位址', port: '連接埠' },
    target: { title: '選擇業務站點', siteName: '站點名稱', listenAddress: '監聽位址', listenPort: '監聽連接埠', protocol: '通訊協定', selectable: '可選擇的受管目標', notSelectable: '不可選擇', unavailableReason: '不可選擇原因', missingValue: '未提供', reasons: { managedTargetInactive: '這個受管目標已停用。', workflowCapabilityMissing: '這個目標不具備目前平台所需的工作流程執行能力。', targetEndpointMissing: '這個目標缺少完整的監聽位址、連接埠或通訊協定。', unknown: '這個目標目前不符合選擇條件。' } }, certificate: { title: '選擇憑證版本', asset: '憑證資產', version: '憑證版本', requiredFormat: '該平台需要 {formats} 格式憑證' },
    ...
    messages: { requestFailed: '接入請求失敗，請檢查權限與輸入。', noPlatforms: '目前沒有可用業務平台。' }
  },
