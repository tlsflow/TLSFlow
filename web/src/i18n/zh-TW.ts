...
export default {
  ...
  internalCa: {
    ...
    adcsAgent: { title: '連接現有 Microsoft AD CS', description: '在已安裝並設定 AD CS 的 Windows Server 上部署 GCAC Adapter；不會安裝 AD CS，也不會建立新的 CA。', providerCount: '目前已設定 {count} 套 Microsoft AD CS 連線', addProvider: '連接現有 AD CS', wizardTitle: '連接現有 Microsoft AD CS', wizardDescription: '為一台已執行憑證授權單位的 AD CS 伺服器建立獨立連線，並產生 Adapter 安裝命令。', adapterRoleTitle: '這是連線適配器，不是 CA 安裝器', adapterRoleDescription: '命令只會安裝 GCAC AD CS Agent。Agent 會探索本機現有 CA，並透過 certreq、certutil 和 Windows 介面執行管理工作；不會安裝 Windows AD CS 角色、初始化根 CA 或建立新的簽發 CA。', requirementsTitle: '目標伺服器必須已符合', requirementInstalled: '已安裝 Active Directory Certificate Services 的 Certification Authority 角色服務。', requirementConfigured: '已完成 Enterprise CA 或 Standalone CA 初始化，並存在作用中的 CA Config。', requirementService: 'CertSvc 服務正在執行。', requirementPermission: '執行安裝命令及執行 Agent 的帳戶具備所需的範本申請、CA 查詢和管理權限。', connectionName: 'AD CS 連線名稱', coexistenceTitle: '與其他 GCAC Agent 共存', coexistenceDescription: 'AD CS Agent 使用獨立服務名稱和獨立目錄，只主動連接 GCAC 控制面，不監聽本機連接埠。它可與裝置管理 Agent 或 Gateway Agent 安裝在同一台 Windows Server 上。', createCommand: '建立連線並產生 Agent 安裝命令', commandReadyTitle: 'Adapter 安裝命令已產生', commandReadyDescription: '已建立 {name}，請在執行現有 AD CS CA 的伺服器上，以系統管理員 PowerShell 執行以下命令。', expiresAt: '命令到期時間：{time}', deleteProvider: '刪除 Provider', deletingProvider: '刪除中…', deleteConfirmText: 'DELETE', deleteProviderRisk: '將刪除此 Provider 及其未完成的註冊權杖、Agent 節點、工作和能力記錄。已綁定憑證機構的 Provider 不允許刪除。', deleteProviderBlocked: '此 Provider 已被憑證機構使用，必須先遷移或移除對應憑證機構。', defaultProviderName: 'Microsoft AD CS Agent', defaultProviderNameIndexed: 'Microsoft AD CS 連線 {index}' },
