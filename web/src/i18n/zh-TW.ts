...
export default {
  ...
  internalCa: {
    ...
    profiles: { recordsTitle: '證書 Profile 記錄', columns: { securityDomain: '安全域', versionCount: '版本數' }, empty: '尚無證書 Profile 記錄。', modalTitle: '新增證書 Profile', modalDescription: '建立可重複使用的 HTTPS 憑證規則；建立後在憑證機構中關聯並生效。', applicableDomains: '適用網域', applicableDomainsPlaceholder: '*.example.com, *.example.org', applicableDomainsHint: '留空表示不限制；填寫的網域同時用於 Profile 匹配和 DNS 後綴驗證。' },
