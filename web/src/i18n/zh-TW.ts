...
export default {
  ...
  internalCa: {
    ...
    sections: { trustDomain: '建立 CA 信任域', provider: 'CA Provider', providerSettings: 'CA Provider 設定', authorityWizard: 'CA 建立精靈', authorityOverview: '憑證機構架構', authorityOverviewDescription: '每張卡片代表一個根信任錨點，選取卡片可查看其下級簽發架構。', caArchitecture: 'CA 層級架構', riskSummary: '安全決策摘要', profile: '建立憑證 Profile', request: '建立應用憑證申請', revocation: '建立撤銷工作', trust: '建立信任散發工作', remediation: '改善預覽' },
    fields: { name: '名稱', code: '唯一代碼', purpose: '用途', isolationLevel: '隔離等級', defaultTrustDomain: '設為預設信任域', trustDomain: 'CA 信任域', parentAuthority: '父根 CA', authorityType: '憑證機構類型', providerType: 'Provider 類型', deploymentMode: '部署模式', platform: '執行平台', provider: 'CA Provider', commonName: 'Common Name', securityDomain: '安全域', topology: 'CA 拓撲', dnsSuffixes: '允許的 DNS 後綴', validityDays: '最大有效期（天）', renewalDays: '提前續期（天）', requireApproval: '簽發前需要審批', applicationAssetId: '應用資產 ID', authority: '憑證機構', profileVersionId: 'Profile 版本 ID', sans: 'SAN 清單', custodyMode: '金鑰託管模式', certificateVersionId: '憑證版本 ID', reason: '撤銷原因', targetIds: '目標 ID 清單' },
    actions: { refresh: '重新整理', addAuthority: '新增 CA', addIntermediate: '新增中繼 CA', previous: '上一步', next: '下一步', createTrustDomain: '建立信任域', createProvider: '建立 Provider', previewRisk: '預覽風險', createAuthority: '建立 CA', createProfile: '建立 Profile', createRequest: '提交申請', approve: '審批通過', retry: '重試', queryResult: '查詢結果', scanRenewals: '掃描到期續期', createRevocation: '建立撤銷工作', createTrust: '建立信任散發', previewRemediation: '預覽改善' },
    placeholders: { dnsSuffixes: 'example.com, office.example.com', sans: 'oa.example.com, 10.0.0.10' },
    messages: { loadFailed: '內部 CA 資料載入失敗。', actionFailed: '操作失敗，請檢查輸入、權限和審批狀態。', noIntermediate: '此根 CA 尚未設定中繼憑證機構。', noRootAuthority: '尚未設定根 CA', noRootAuthorityDescription: '新增根 CA 以建立第一套獨立信任架構。', trustDomainCreated: 'CA 信任域已建立。', providerCreated: 'CA Provider 已建立。', authorityCreated: '憑證機構已建立。', profileCreated: '憑證 Profile 已建立。', requestCreated: '憑證申請已提交。', requestApproved: '憑證申請已審批。', requestRetried: '憑證簽發已重試。', requestQueried: '遠端簽發結果已重新整理。', renewalScanned: '續期掃描已完成。', revocationCreated: '撤銷工作已建立並等待審批。', revocationApproved: '憑證撤銷已審批。', trustCreated: '信任散發工作已建立並等待審批。', trustApproved: '信任散發已審批。' },
    metrics: { nodes: 'CA Node', renewals: '續期工作', revocations: '撤銷工作', trust: '信任散發', totalRisks: '風險總數', critical: '嚴重風險', affectedAssets: '受影響應用資產' },
    labels: { rootAuthority: '根憑證機構', intermediateAuthority: '中繼憑證機構', intermediateCount: '{count} 個中繼 CA', expiresAt: '到期時間：{time}', defaultTrustDomain: '預設信任域', independentTrustDomain: '獨立根信任邊界', trustDomainCount: '{count} 個 CA 信任域', versionCount: '{count} 個版本', assetCount: '{count} 個應用資產', requestCount: '將建立 {count} 個獨立憑證申請' },
    wizard: { title: '新增憑證機構', description: '建立新的根信任架構，或在現有根 CA 下新增中繼憑證機構。', stepsAria: 'CA 建立步驟', step1: '選擇類型', step2: '填寫設定', step3: '確認風險', rootTitle: '根 CA', rootDescription: '建立新的獨立根信任錨點，並可同時建立首個中繼 CA。', intermediateTitle: '中繼 CA', intermediateDescription: '掛載至現有根 CA 下承擔日常簽發，不建立新的根信任邊界。', noWarnings: '未發現額外的拓撲風險警告。' },
