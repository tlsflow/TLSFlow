...
export default {
  ...
  tasks: {
    ...
    values: { system: '시스템', externalApi: '외부 API', empty: '기록 없음', none: '없음' },
    agentUpdate: {
    ...
    summaryTemplates: {
      WAITING_RESULT_COUNTDOWN: '배포 시간을 기다리는 중, 카운트다운 {countdown}',
      QUEUED: '{task} 대기열 등록됨',
      RUNNING: '{task} 실행 중',
  ...
  automations: {
    deploymentSchedule: { immediate: '즉시 업데이트', scheduled: '지정 시간에 업데이트', help: '인증서 새 버전 후 다음 지정 시간에 실행' },
    title: '자동화',
    ...
    triggers: { onDemand: '요청 시', onDemandDescription: '플랫폼 내 실행 작업으로만 시작됩니다. 외부 API 키는 생성되지 않습니다.', schedule: '예약' },
    triggerTypes: { on_demand: '요청 시', schedule: '예약', certificate_version_created: 'Certificate new-version event', retry: '실패 대상 재시도' },
    eventSources: { acme_issue: 'ACME 자동 갱신', manual_import: '수동 가져오기' },
    ...
    preview: { title: '자산 영향 미리 보기', description: '각 행은 하나의 관리 애플리케이션을 나타냅니다. 가운데에는 현재 인증서 만료 시각에서 선택한 인증서 만료 시각으로의 변화가, 오른쪽에는 실행 가능 여부가 표시됩니다.', explanation: '영향 분석에는 각 관리 애플리케이션의 현재 인증서 만료 시각, 대상 인증서 만료 시각 및 실행 결과가 표시됩니다.', applicationAsset: '애플리케이션 자산', certificate: '인증서', applicationAssetId: '애플리케이션 자산 ID', bindingId: '바인딩 ID', missingCurrentExplanation: '“날짜 없음 → 대상 날짜”는 현재 연결된 인증서의 만료 시각을 읽지 못해 이 대상이 업데이트되지 않음을 의미합니다. 인증서 바인딩 또는 현재 인증서 정보를 먼저 보완하세요.', matched: '{count}개 일치', executable: '{count}개 실행 가능', excluded: '{count}개 제외', affected: '{count}개 영향', upgrade: '{count}개 유효기간 연장', same: '{count}개 만료 시각 동일', skip: '{count}개 업데이트 건너뜀', downgrade: '{count}개 확인 필요', version: '버전 {version}', versionUnknown: '버전 알 수 없음', ready: '실행 가능', skipUpdate: '업데이트 건너뛰기', expiryLabel: '만료', impact: { upgrade: '유효기간 연장', same: '만료 시각 동일', downgrade: '유효기간 단축 위험', missing_current: '현재 인증서 없음', unknown: '영향 알 수 없음' } },
  ...
  settings: {
    ...
    version: {
      eyebrow: '버전 거버넌스',
      title: '버전 정보',
      description: '확인할 채널을 선택하고 호스트 수동 업데이트 안내를 확인합니다.',
      currentVersion: '실행 중인 버전',
      product: '제품',
      channelLabel: '업데이트 채널',
      channelDescription: '이 페이지는 버전만 확인하며 Docker, 업데이트 또는 롤백을 실행하지 않습니다.',
      readonly: '이 계정은 읽기 전용입니다.',
      channels: { stable: '안정 버전', stableDescription: '운영 환경용이며 검증된 버전을 우선합니다.', dev: '개발 버전', devDescription: '개발 빌드의 조기 검증용이며 운영 환경에는 권장하지 않습니다.' },
      checkTitle: '업데이트 확인',
      checkDescription: '선택한 채널의 구체적인 버전을 확인하고 호스트에서 복사할 명령을 생성합니다.',
      checkEmpty: '아직 업데이트를 확인하지 않았습니다.',
      lastChecked: '확인 시간: {time}',
      targetVersion: '대상 버전',
      publishedAt: '게시 시간',
      releaseNotes: '업데이트 설명',
      manualTitle: '수동 업데이트 안내',
      manualDescription: '배포 호스트에서 다음 방법 중 하나를 실행하세요. 웹 페이지가 대신 실행하지 않습니다.',
      installScriptCommand: 'install.sh',
      composeCommand: 'Docker Compose',
      relations: { current: '선택한 채널 버전이 이미 실행 중입니다', upgrade: '업데이트 가능', downgrade: '대상 버전이 현재 버전보다 낮습니다', channel_switch: '확인 채널이 변경되었습니다' },
      status: { current: '최신 버전입니다' },
      actions: { save: '채널 저장', saving: '저장 중…', check: '업데이트 확인', checking: '확인 중…' },
      messages: { channelSaved: '업데이트 채널을 저장했습니다.' },
      errors: { emptySettings: '백엔드가 업데이트 채널 설정을 반환하지 않았습니다.', emptyCheck: '백엔드가 확인 결과를 반환하지 않았습니다.', loadFailed: '업데이트 채널을 불러오지 못했습니다.', saveFailed: '업데이트 채널을 저장하지 못했습니다.', checkFailed: '업데이트 확인에 실패했습니다.' }
    },
    permissionPolicies: {
      resourceName: '권한 정책',
      actions: {
  ...
  applicationOnboarding: {
    ...
    resource: { title: '플랫폼 리소스 선택', refresh: '리소스 새로고침', loading: '사용 가능한 리소스를 불러오는 중입니다.', empty: '사용 가능한 플랫폼 리소스가 없습니다.' },
    ...
    automation: { defaultName: '애플리케이션 인증서 자동 업데이트', defaultDescription: '애플리케이션 온보딩 마법사에서 생성한 인증서 업데이트 자동화입니다.' },
    complete: { title: '연결 완료', description: '애플리케이션 자산과 배포 계획이 생성되었습니다.', progressAria: '생성 진행 상황', viewSummary: '구성 요약 보기', stages: { application: '애플리케이션 생성', certificate: '인증서 연결', automation: '자동화 생성' }, status: { pending: '대기 중', running: '진행 중', success: '완료', failed: '실패' } },
