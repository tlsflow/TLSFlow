...
export default {
  ...
  automations: {
    ...
    errors: { loadFailed: '自動化一覧の読み込みに失敗しました', applicationAssetsLoadFailed: 'Failed to load application assets. Try again later.' }
  },
  routes: {
  ...
  applicationOnboarding: {
    ...
    platforms: { customManual: 'カスタム手動作成', manualHint: '従来の手動フローを使用', pluginHint: 'プラグインの固定フローを使用', capabilityVersion: '接続機能バージョン', compatibility: '対応プラットフォームバージョン', requiredInformation: '事前に必要な情報', inReview: '機能検証中のため、現在は利用できません' },
    device: { title: 'プラットフォームに接続', existing: '既存デバイスを使用', new: 'デバイスを追加', deviceId: 'デバイス ID', selectPlaceholder: 'デバイスを選択', noExisting: 'このプラットフォームに使用できる正常なデバイスはありません。', existingLoading: '互換デバイスを読み込んでいます。', refreshExisting: 'デバイスを更新', newDescription: '統一デバイス登録ウィザードを開き、Agent 登録またはデバイス登録後にこのウィザードへ戻ります。', newHint: '戻ると、このプラットフォームで検出済みの利用可能なデバイスを再読み込みします。', newAction: 'デバイス登録を開く', username: 'ユーザー名', password: 'パスワード', host: 'アドレス', port: 'ポート' },
    target: { title: 'サイトを選択', siteName: 'サイト名', listenAddress: 'リッスンアドレス', listenPort: 'リッスンポート', protocol: 'プロトコル', selectable: '選択可能な管理対象', notSelectable: '選択不可', unavailableReason: '選択できない理由', missingValue: '未提供', reasons: { managedTargetInactive: 'この管理対象は無効になっています。', workflowCapabilityMissing: 'この対象には、現在のプラットフォームに必要なワークフロー実行機能がありません。', targetEndpointMissing: 'この対象には、完全なリッスンアドレス、ポート、またはプロトコルがありません。', unknown: 'この対象は現在、選択条件を満たしていません。' } }, certificate: { title: '証明書バージョンを選択', asset: '証明書資産', version: '証明書バージョン' },
    ...
    messages: { requestFailed: '接続に失敗しました。権限と入力を確認してください。', noPlatforms: '利用可能なプラットフォームはありません。' }
  },
