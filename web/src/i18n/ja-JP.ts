...
export default {
  ...
  automations: {
    ...
    fields: { name: '名前', description: '説明', trigger: 'トリガー', cron: 'Cron 式', timeZone: 'タイムゾーン', expiresWithinDays: '有効期限までの日数', environments: '対象環境（カンマ区切り）', maxTargets: '1 回の最大対象数', concurrency: '同時実行数', failureCount: '失敗件数しきい値', requireDryRun: '実行前に Dry Run を必須にする', requireApproval: '実行前に承認を必須にする', startedAt: '開始時刻', finishedAt: '終了時刻', failureStage: '失敗段階', parentRun: '親実行' },
    ...
    editor: { createTitle: '自動化を作成', editTitle: '自動化を編集', description: 'トリガー、対象選択、アクション、安全ガードレールを設定します。' },
