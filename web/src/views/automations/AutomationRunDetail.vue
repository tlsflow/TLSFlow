...
<template>
  <section v-if="run" class="run-detail">
    ...
    <dl class="run-detail__facts"><div><dt>{{ t('automations.fields.startedAt') }}</dt><dd>{{ formatMaybeLocalTime(run.startedAt, t('automations.common.notAvailable')) }}</dd></div><div><dt>{{ t('automations.fields.finishedAt') }}</dt><dd>{{ formatMaybeLocalTime(run.finishedAt, t('automations.common.notAvailable')) }}</dd></div><div><dt>{{ t('automations.fields.failureStage') }}</dt><dd>{{ run.failureStage ? t(`automations.failureStages.${run.failureStage}`) : t('automations.common.notAvailable') }}</dd></div><div><dt>{{ t('automations.fields.parentRun') }}</dt><dd>{{ run.parentRunId || t('automations.common.notAvailable') }}</dd></div></dl>
...
.run-detail__facts div, .run-targets article { padding: var(--gc-space-4); border: var(--gc-border-width-default) solid var(--gc-color-border-muted); border-radius: var(--gc-radius-md); background: var(--gc-color-surface-raised); }
