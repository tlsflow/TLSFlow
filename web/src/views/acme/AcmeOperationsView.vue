...
const failedJobs = computed(() => jobs.value.filter((job) => ['failed', 'rollback_required', 'blocked'].includes(text(job.status))))
const dueJobs = computed(() => jobs.value.filter((job) => ['scheduled', 'retry_waiting', 'deploying', 'verifying', 'issuing'].includes(text(job.status))))
...
const assetColumns = computed<DataTableColumn<ApiRecord>[]>(() => [
  ...
  { key: 'installation', title: t('acme.list.columns.installation') },
...
const detailJobColumns = computed<DataTableColumn<InternalCaRecord>[]>(() => [
  ...
  { key: 'promotionStatus', title: t('acme.detail.columns.promotion') },
...
function statusLabel(value: unknown): string {
  return t(`acme.statuses.${text(value, 'unknown')}`)
}
...
function installationLabel(asset: ApiRecord): string {
  const policy = policyForAsset(asset)
  if (!policy) return t('acme.list.values.notConfigured')
  return text(policy.deploymentMode) === 'automatic' ? t('acme.list.values.automatic') : t('acme.list.values.approval')
}
...
<template>
  <section class="acme-page">
    ...
    <GcDataTable :columns="assetColumns" :rows="configuredAssets" :loading="loading" :empty-text="t('acme.list.empty')">
      ...
      <template #cell-installation="{ row }">{{ installationLabel(row) }}</template>
      <template #cell-actions="{ row }">
        <div class="acme-row-actions">
          <button class="gc-button gc-button--compact" type="button" :disabled="actionPending || !policyForAsset(row)" @click="toggleRenewal(row)">{{ renewalLabel(row) === t('acme.list.values.enabled') ? t('acme.actions.disable') : t('acme.actions.enable') }}</button>
      ...
      <form class="acme-form" @submit.prevent="createAcmeCertificate">
        ...
        <section v-if="createDraft.challengeType === 'dns-01'" class="acme-dns-fields">
          ...
          <div v-if="selectedDnsProvider" class="acme-provider-meta">
            ...
            <span>{{ text(selectedDnsProvider.fullPluginName) }}</span>
          </div>
...
.acme-status[data-status='issuing'], .acme-status[data-status='deploying'], .acme-status[data-status='verifying'], .acme-status[data-status='retry_waiting'] { color: var(--gc-color-warning); background: var(--gc-color-warning-bg); }
