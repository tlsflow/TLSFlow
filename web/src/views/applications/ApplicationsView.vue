...
import { formatBrowserLocalTime, formatMaybeLocalTime, getExpiryCountdown } from '@/utils/browser-local-time'
...
} from '../assets/asset-deployment-strategy.model'
import {
...
import {
  ...
  readInputBindingsV1,
} from '../assets/asset-input-bindings.model'
...
type CertificateDeploymentSelection = { certificateAssetId: string; selectionMode: 'EXPLICIT' | 'LATEST_AUTO'; certificateVersionId: string; reapply?: boolean }
...
const editInitializationLoading = ref(false)
...
const deploymentCertificateSupplyData = ref<CertificateSupplyData | null>(null)
const deploymentCertificateSupplyResolved = ref(false)
...
let workflowProjectionRequestKey = ''
...
const certificateSupplyProfileOptions = computed(() => {
  const authorityId = certificateSupplyDraft.certificateAuthorityId
  const providerId = certificateSupplyDraft.providerId
  const profiles = certificateSupplyProviders.value.profiles ?? []
  return profiles.flatMap((profile) => {
    if (String(profile.status ?? 'active') !== 'active') return []
    if (authorityId && String(profile.certificateAuthorityId ?? '') !== authorityId) return []
    if (providerId && String(profile.providerId ?? '') !== providerId) return []
    const versions = Array.isArray(profile.versions) ? profile.versions : []
    return versions.map((version) => ({
      id: String(version.id ?? ''),
      label: `${String(profile.name ?? profile.id)} v${String(version.versionNo ?? '')}`,
      isCurrent: Number(version.versionNo) === Number(profile.currentVersion),
    })).filter((item) => item.id)
  }).sort((left, right) => Number(right.isCurrent) - Number(left.isCurrent) || left.label.localeCompare(right.label))
})
...
const deploymentCertificateSupplyVersion = computed<ApiRecord | null>(() =>
  deploymentCertificateSupplyData.value?.currentVersion ?? null,
)

const deploymentCurrentCertificateVersionId = computed(() => firstAssetText(
  deploymentCertificateSupplyVersion.value ?? deploymentBinding.value ?? deploymentApplicationAsset.value ?? {},
...
const deploymentCertificateAssetId = computed(() => {
  // 部署目标必须来自应用资产当前证书供应策略，不能让全量证书资产参与选择。
  if (deploymentCertificateSupplyResolved.value) {
    const current = deploymentCertificateSupplyVersion.value ?? {}
    const supplyMode = String(current.supplyMode ?? '').toLowerCase()
    const directId = firstAssetText(current, ['certificateAssetId', 'certificateId'])
    if (directId) return directId
    if (supplyMode === 'manual') {
      const versionId = firstAssetText(current, ['certificateVersionId'])
      const candidate = deploymentCertificateSupplyData.value?.certificateCandidates?.find((item) =>
        firstAssetText(item, ['certificateVersionId']) === versionId,
      )
      const candidateId = firstAssetText(candidate ?? {}, ['certificateAssetId', 'certificateId'])
      if (candidateId) return candidateId
      return firstAssetText(deploymentBinding.value ?? {}, [
        'certificateAssetId',
        'certificateId',
    ...
    if (supplyMode === 'dedicated') return ''
    // 兼容尚未写入供应策略的历史应用资产，继续从现有绑定解析证书资产。
    return firstAssetText(deploymentBinding.value ?? {}, [
      'certificateAssetId',
      'certificateId',
      'certificate.certificateAssetId',
...
const deploymentCertificate = computed<ApiRecord | null>(() => {
  ...
  const selectedVersion = deploymentCertificateVersionItems.value.find((item) =>
    firstAssetText(item, ['id', 'certificateVersionId']) === deploymentCurrentCertificateVersionId.value,
  )
  if (!assetId && !current && !selectedVersion) return null
  return {
    id: assetId,
    primaryDomain: firstAssetText(current ?? selectedVersion ?? {}, ['commonName', 'subject.commonName', 'primaryDomain', 'name']),
    commonName: firstAssetText(current ?? selectedVersion ?? {}, ['commonName', 'subject.commonName', 'primaryDomain']),
  } as ApiRecord
})
...
const deploymentCertificateVersions = computed<ApiRecord[]>(() => {
  const assetId = deploymentCertificateAssetId.value
  if (!assetId) return []
  return deploymentCertificateVersionItems.value.filter((item) =>
    firstAssetText(item, ['certificateAssetId', 'certificateId']) === assetId,
  )
})

const deploymentCertificateSupplyMode = computed<ApplicationCertificateSupplyMode>(() =>
  String(deploymentCertificateSupplyVersion.value?.supplyMode ?? '').toLowerCase() === 'dedicated' ? 'dedicated' : 'manual',
)
...
const deploymentDedicatedDetails = computed(() => {
  const version = deploymentDedicatedVersion.value
  const current = deploymentCertificateSupplyVersion.value
  const providerType = firstAssetText(current ?? {}, ['providerType']).toLowerCase() as 'acme' | 'internal_ca' | ''
  const providers = deploymentCertificateSupplyData.value?.providers
  const authorityId = firstAssetText(current ?? {}, ['certificateAuthorityId'])
  const authority = providers?.authorities?.find((item) => String(item.id ?? '') === authorityId)
  const provider = [...(providers?.ca ?? []), ...(providers?.acme ?? [])].find((item) => String(item.id ?? '') === firstAssetText(current ?? {}, ['providerId']))
  const expiresAt = firstAssetText(version ?? {}, ['notAfter', 'validTo', 'expiresAt'])
  const issuedAt = firstAssetText(version ?? {}, ['createdAt', 'notBefore', 'validFrom', 'issuedAt'])
...
const executionCompatibilityIssues = computed<DeploymentInputIssueDetail[]>(() => {
  const records = readNested(editAssetDetail.value, ['executionCompatibility'])
  if (!Array.isArray(records)) return []
  return records.flatMap((record) => {
    const current = readRecord(record)
    if (!current || current.status === 'READY') return []
    const issues = Array.isArray(current.issues) ? current.issues : []
    return issues.filter((issue): issue is DeploymentInputIssueDetail => Boolean(issue && typeof issue === 'object'))
  })
})
...
async function loadApplicationAssetContext(row: ViewRow) {
...
async function openDeploymentDialog(row?: ViewRow) {
  ...
  deploymentCertificateSupplyData.value = null
  deploymentCertificateSupplyResolved.value = false
  deploymentCertificateVersionItems.value = []
...
async function loadDeploymentDialogOptions() {
  ...
  try {
    ...
    let certificateAssetId = firstAssetText(currentVersion, ['certificateAssetId', 'certificateId'])
    if (!certificateAssetId && String(currentVersion.supplyMode ?? '').toLowerCase() === 'manual') {
      const currentVersionId = firstAssetText(currentVersion, ['certificateVersionId'])
      const candidate = deploymentCertificateSupplyData.value.certificateCandidates?.find((item) =>
        firstAssetText(item, ['certificateVersionId']) === currentVersionId,
      )
      certificateAssetId = firstAssetText(candidate ?? {}, ['certificateAssetId', 'certificateId'])
    }
    if (!certificateAssetId && String(currentVersion.supplyMode ?? '').toLowerCase() !== 'dedicated') {
    ...
    if (certificateAssetId) {
      try {
        listedVersions = await fetchAllRecords((page, pageSize) => listCertificateVersions({
          ...
          pageSize,
          sort: 'createdAt:desc',
          filters: { certificateAssetId },
        }))
      } catch (cause) {
        // 版本列表有独立的证书对象权限；供应策略候选仍是后端按应用授权返回的可部署集合。
        versionListError = cause
      }
    }

    const candidateVersions = supplyMode !== 'dedicated'
      ? certificateSupplyCandidateVersionItems(deploymentCertificateSupplyData.value, certificateAssetId)
      : []
    const hasDeployableListedVersion = sortDeployableCertificateVersions(listedVersions).length > 0
    const currentVersionId = firstAssetText(currentVersion, ['certificateVersionId'])
    const listedCurrentVersion = currentVersionId
      ? listedVersions.some((item) => firstAssetText(item, ['id', 'certificateVersionId']) === currentVersionId)
      : true
    // 历史手动策略可能只能从供应策略接口读到候选版本，或者版本列表中只剩归档/过期版本。
    // 候选已经经过后端的域名、颁发者和可部署性筛选，应作为手动部署的可靠回退来源。
    if ((supplyMode === 'manual' || !supplyMode) && (!hasDeployableListedVersion || versionListError || !listedCurrentVersion) && candidateVersions.length > 0) {
      const versionsById = new Map(listedVersions.map((item) => [firstAssetText(item, ['id', 'certificateVersionId']), item]))
      for (const candidate of candidateVersions) {
        const id = firstAssetText(candidate, ['id', 'certificateVersionId'])
        if (id && !versionsById.has(id)) versionsById.set(id, candidate)
      }
      deploymentCertificateVersionItems.value = [...versionsById.values()]
    } else {
      deploymentCertificateVersionItems.value = listedVersions
    }

    if (versionListError && deploymentCertificateVersionItems.value.length === 0) {
      throw versionListError
    }
  } catch (cause) {
    deploymentCertificateSupplyData.value = null
    deploymentCertificateSupplyResolved.value = true
...
function certificateSupplyCandidateVersionItems(data: CertificateSupplyData, certificateAssetId: string): ApiRecord[] {
  return (data.certificateCandidates ?? [])
    .filter((candidate) => {
      const candidateAssetId = firstAssetText(candidate, ['certificateAssetId', 'certificateId', 'certificateAsset.id', 'certificate.id'])
      return Boolean(certificateAssetId) && candidateAssetId === certificateAssetId
    })
    .map((candidate) => {
      const versionId = firstAssetText(candidate, ['certificateVersionId', 'id'])
      return {
        ...candidate,
        id: versionId,
        certificateAssetId,
        status: firstAssetText(candidate, ['status']) || 'active',
        deployable: candidate.deployable !== false,
      }
    })
    .filter((candidate) => Boolean(firstAssetText(candidate, ['id', 'certificateVersionId'])))
}
...
): Promise<'STARTED' | 'PENDING' | 'DEDICATED'> {
  if (!applicationAssetId) {
    throw new Error(t('assets.deployment.errors.missingApplicationAssetId'))
  }
  let certificateVersionId = selection.certificateVersionId
  let certificateAssetId = selection.certificateAssetId
  const supplyData = applicationAssetId === selectedApplicationAssetId.value
    ? deploymentCertificateSupplyData.value
    : ((await getApplicationCertificateSupplyPolicy(applicationAssetId)).data ?? {}) as CertificateSupplyData
  const supplyVersion = supplyData?.currentVersion ?? null
  const supplyMode = String(supplyVersion?.supplyMode ?? '').toLowerCase()
  if (supplyMode === 'dedicated') {
    const queued = await enqueueApplicationCertificateDeployment(applicationAssetId, selection.reapply === true)
    if (!queued.data?.id) throw new Error(t('assets.deployment.errors.createTaskMissingId'))
    return 'DEDICATED'
  }
  if (!certificateVersionId) {
    throw new Error(t('assets.deployment.errors.missingCertificateVersion'))
  }
...
async function deployCertificateVersion(selection: CertificateDeploymentSelection) {
  ...
  try {
    ...
    let startedCount = 0
    let dedicatedQueued = false

    for (const applicationAssetId of targetAssetIds) {
      try {
        const result = await runCertificateDeployment(applicationAssetId, selection, settings, !isBulk)
        if (result === 'STARTED') startedCount += 1
        if (result === 'DEDICATED') {
          startedCount += 1
          dedicatedQueued = true
        }
      } catch (cause) {
    ...
    if (!isBulk) {
      if (startedCount === 0 && deploymentInfo.value) return
      ...
      if (!dedicatedQueued) await loadDeploymentRecords(selectedApplicationAssetId.value)
      return
    }
...
function certificateSupplyPayload(): Record<string, unknown> {
  ...
  if (certificateSupplyDraft.providerType === 'internal_ca' && certificateSupplyDraft.certificateProfileVersionId) payload.certificateProfileVersionId = certificateSupplyDraft.certificateProfileVersionId
...
function selectInternalCaAuthority(authorityId: string): void {
  const authority = certificateSupplyAuthorities.value.find((item) => String(item.id ?? '') === authorityId)
  if (authority?.providerId) certificateSupplyDraft.providerId = String(authority.providerId)
  const current = certificateSupplyProfileOptions.value.find((item) => item.isCurrent)
  if (!certificateSupplyDraft.certificateProfileVersionId || !certificateSupplyProfileOptions.value.some((item) => item.id === certificateSupplyDraft.certificateProfileVersionId)) {
    certificateSupplyDraft.certificateProfileVersionId = current?.id ?? certificateSupplyProfileOptions.value[0]?.id ?? ''
  }
}
...
async function openEditDialog(row: ViewRow) {
  ...
  // 证书供应策略属于编辑后的增强信息，不得阻塞资产基础字段首屏。
  void loadApplicationCertificateSupplyPolicy(editingServiceAssetId.value)
  ...
  try {
    ...
    if (assetDraft.managementMode === 'WORKFLOW') {
      ...
      assetDraft.workflowPluginId = String(readNested(deploymentStrategy, ['workflow', 'pluginId']) ?? '')
    ...
    await Promise.all([
      loadDevices(),
      loadServiceInstances(assetDraft.deviceId),
      refreshAssetTargets(),
      loadManagedTargets(assetDraft.siteAssetId, contextManagedTarget ?? undefined),
      loadWorkflowTemplates(),
      loadGateways(),
      loadCredentialsForWorkflowVariables(),
      loadCertificateFormatsForWorkflow(),
...
async function loadExistingWorkflowExecutionBinding(bindingId: string) {
  ...
  assetDraft.workflowPluginId = String(binding.pluginId ?? '')
...
async function refreshWorkflowBindingProjection(options: { preserveRenderedForm?: boolean } = {}) {
  ...
  const requestKey = JSON.stringify({
    ...
    workflowVersionId: assetDraft.workflowVersionId,
    applicationAssetId: editingServiceAssetId.value,
    inputBindings: deploymentInputBindingsFingerprint.value,
    address: assetDraft.address,
    port: assetDraft.port,
    protocol: assetDraft.protocol,
    certificateFormatId: assetDraft.agentCertificateFormatId,
  })
  if (requestKey === workflowProjectionRequestKey && (workflowProjectionLoading.value || workflowBindingProjection.value)) return
  workflowProjectionRequestKey = requestKey
...
async function refreshAssetDetail(serviceAssetId: string) {
  ...
  try {
    const result = await getApplicationDetail(serviceAssetId)
...
function buildWorkflowExecutionInput(): Record<string, unknown> {
  ...
  return {
    ...
    ...(identity.pluginId ? { pluginId: identity.pluginId } : {}),
...
function resolveWorkflowExecutionIdentity(): { pluginId: string; capabilityKey: string; workflowKey: string } {
  const pluginSource = readRecord(readNested(selectedWorkflowVersion.value, ['pluginSource']))
  ...
  const pluginId = String(pluginSource?.pluginId ?? assetDraft.workflowPluginId).trim()
  const workflowKey = String(assetDraft.workflowKey || capabilityKey).trim()
  return { pluginId, capabilityKey, workflowKey }
}
...
function buildDeploymentStrategyPayload(workflowTarget?: WorkflowTargetInfo): Record<string, unknown> {
  if (assetDraft.managementMode === 'WORKFLOW') {
    ...
    return {
      ...
      workflow: {
        pluginBindingId: pluginBindingId.value || undefined,
        pluginId: identity.pluginId || undefined,
...
<template>
  <section class="asset-page">
    ...
    <GcModal
      ...
      size="xxl"
    >
      <section v-if="selectedServiceAsset" class="application-detail-modal">
        <header class="application-detail-modal__hero">
          <div class="application-detail-modal__hero-copy">
            <p class="application-detail-modal__eyebrow">{{ t('assets.resourceName') }}</p>
          ...
          <div class="application-detail-modal__hero-side">
            <GcStatusTag :status="String(selectedServiceAsset.status)" />
            <div class="application-detail-modal__spotlight">
        ...
        <section v-if="activeDetailTab === 'overview'" class="application-detail-modal__sections">
          <article class="application-detail-modal__section">
            <div class="application-detail-modal__section-head">
            ...
            <div class="application-detail-modal__links">
          ...
          <article class="application-detail-modal__section">
            ...
            <dl class="application-detail-modal__grid">
              <div class="application-detail-modal__item" v-for="field in assetDetailFields" :key="field.label">
                <dt>{{ field.label }}</dt>
                <dd>{{ renderValue(detailFieldValue(field.candidates)) }}</dd>
              </div>
            </dl>
            <nav class="application-detail-modal__links">
              <RouterLink
        ...
        <section v-else class="application-detail-modal__sections">
          <article class="application-detail-modal__section">
            <div class="application-detail-modal__section-head">
    ...
    >
      <section v-if="editInitializationLoading" class="asset-form asset-form--loading" aria-busy="true">
        <p class="asset-form__hint">{{ t('common.loading') }}</p>
      </section>
      <section v-else-if="editInitializationError" class="asset-form asset-form--loading">
        <p class="asset-form__error">{{ editInitializationError }}</p>
      </section>
      <section v-else-if="isUserViewMode" class="asset-form asset-user-form gc-native-select-surface">
        ...
        <section class="asset-certificate-supply" :aria-label="t('assets.certificateSupply.title')">
          ...
          <div v-else class="asset-form__grid">
            ...
            <label v-if="certificateSupplyDraft.providerType === 'internal_ca'" class="asset-form__field">
              ...
              <select v-model="certificateSupplyDraft.certificateAuthorityId" @change="selectInternalCaAuthority(certificateSupplyDraft.certificateAuthorityId)">
            ...
            <label v-if="certificateSupplyDraft.providerType === 'internal_ca' && !isEditMode" class="asset-form__field">
              ...
              <select v-model="certificateSupplyDraft.certificateProfileVersionId">
                ...
                <option v-for="profile in certificateSupplyProfileOptions" :key="profile.id" :value="profile.id">{{ profile.label }}</option>
              </select>
            </label>
      ...
      <section v-else class="asset-form asset-wizard">
        ...
        <section v-else-if="assetWizardStep === 2" class="asset-wizard__panel gc-native-select-surface">
          ...
          <template v-if="workflowExecutionEnabled">
            ...
            <section v-if="executionCompatibilityIssues.length" class="asset-deployment__error-details" :aria-label="t('deploymentInputs.issues.title')">
              <h4>{{ t('deploymentInputs.issues.title') }}</h4>
              <ul>
                <li v-for="(issue, index) in executionCompatibilityIssues" :key="`${String(issue.code ?? 'unknown')}:${String(issue.path ?? issue.slot ?? '')}:${index}`">
                  <strong>{{ deploymentInputIssueLabel(issue) }}</strong>
                  <code>{{ deploymentInputIssuePath(issue) }}</code>
                </li>
              </ul>
      ...
      <template #actions>
        ...
        <button v-if="!isUserViewMode" class="gc-button" type="button" :disabled="createLoading || editInitializationLoading || Boolean(editInitializationError) || !canGoPreviousAssetStep" @click="goPreviousAssetStep">{{ t('designSystem.deploymentWizard.actions.previous') }}</button>
        <button v-if="!isUserViewMode && assetWizardStep < 3" class="gc-button gc-button--primary" type="button" :disabled="createLoading || editInitializationLoading || Boolean(editInitializationError) || !canGoNextAssetStep" @click="goNextAssetStep">{{ t('designSystem.deploymentWizard.actions.next') }}</button>
        <button v-else-if="isUserViewMode || assetWizardStep >= 3" class="gc-button gc-button--primary" type="button" :disabled="createDisabled || editInitializationLoading || Boolean(editInitializationError)" @click="submitCreate">
...
.application-detail-modal__hero {
  display: flex;
  justify-content: space-between;
  align-items: stretch;
  gap: var(--gc-space-4);
...
.application-detail-modal__hero-copy {
  display: grid;
  gap: var(--gc-space-1);
  min-width: 0;
}

.application-detail-modal__eyebrow {
  margin: 0;
  color: var(--gc-color-text-muted);
  font-size: var(--gc-font-size-xs);
  font-weight: 800;
...
.application-detail-modal__hero-copy h2 {
  margin: 0;
  color: var(--gc-color-text);
...
.application-detail-modal__hero-copy span {
  color: var(--gc-color-text-muted);
  font-size: var(--gc-font-size-xs);
  font-weight: 700;
  overflow-wrap: anywhere;
...
.application-detail-modal__hero-side {
  display: grid;
...
.application-detail-modal__spotlight {
  display: grid;
  gap: var(--gc-space-1);
  min-width: calc(var(--gc-size-card-min) - var(--gc-space-7));
...
.application-detail-modal__spotlight small {
...
.application-detail-modal__spotlight strong {
  font-size: var(--gc-font-size-md);
  line-height: 1.15;
  letter-spacing: 0;
  overflow-wrap: anywhere;
...
.application-detail-modal__sections {
  display: grid;
  gap: var(--gc-space-3);
}

.application-detail-modal__section {
  display: grid;
  gap: var(--gc-space-3);
  padding: var(--gc-space-4);
  border: var(--gc-border-width-default) solid var(--gc-color-border-muted);
...
.application-detail-modal__section-head {
  display: grid;
  gap: var(--gc-space-1);
}

.application-detail-modal__section-head h3,
.application-detail-modal__section-head p {
  margin: 0;
}
...
.application-detail-modal__grid {
  display: grid;
...
.application-detail-modal__item {
  display: grid;
  gap: var(--gc-space-1);
...
.application-detail-modal__item dt {
  color: var(--gc-color-text-muted);
  font-size: var(--gc-font-size-xs);
  font-weight: 800;
...
.application-detail-modal__item dd {
  margin: 0;
  color: var(--gc-color-text);
  font-size: var(--gc-font-size-sm);
  line-height: 1.35;
...
.application-detail-modal__links {
  display: flex;
  flex-wrap: wrap;
  gap: var(--gc-space-2);
}
