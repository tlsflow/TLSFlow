...
describe('Spec 033.5 执行来源组件', () => {
  ...
  it('工作流执行表单不提供版本选择', async () => {
    const wrapper = mount(GcWorkflowExecutionForm, {
      props: {
        workflowId: 'workflow-1', runner: 'CONTROL_PLANE', gatewayId: '',
        workflows: [{ id: 'workflow-1', name: 'workflow' }], gateways: [],
        labels: { workflow: 'workflow', workflowPlaceholder: 'select', runner: 'runner', controlPlane: 'control', gateway: 'gateway', gatewayPlaceholder: 'select gateway' },
      },
    })
  ...
  it('插件来源选择按工作流合并，再按能力选择当前来源', async () => {
    ...
    expect(wrapper.findAll('.gc-plugin-workflow-source-selector__capability')).toHaveLength(0)
    ...
    expect(wrapper.findAll('.gc-plugin-workflow-source-selector__capability')).toHaveLength(2)
    ...
    await wrapper.findAll('.gc-plugin-workflow-source-selector__capability')[0]!.trigger('click')
