...
describe('应用与证书产物视图', () => {
  ...
  it('专业应用页按 CertFlow 卡片布局支持多选，详情仍从卡片操作可达', async () => {
    ...
    expect(card.find('.asset-page__card-fact-value .gc-tag').text()).toBe('需配置')
    expect(card.find('.asset-page__card-fact-value .gc-tag').classes()).toContain('gc-tag--warning')
    expect(card.text()).not.toContain('common.status.UNKNOWN')
