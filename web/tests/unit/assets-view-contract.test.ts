...
describe('应用资产卡片契约', () => {
  ...
  it('按 CertFlow 卡片布局提供多选和三栏证书事实区', () => {
    ...
    expect(source).toContain("'assets.actions.latestCertificate'")
  ...
  it('资产证书更新使用单页版本选择并在发起后回到任务列表', () => {
    ...
    expect(source).toContain("type CertificateDeploymentSelection = { certificateAssetId: string; selectionMode: 'EXPLICIT' | 'LATEST_AUTO'; certificateVersionId: string }")
    expect(source).toContain('certificateAssetId: selection.certificateAssetId')
    expect(source).toContain('targetCertificateVersionId: selection.certificateVersionId')
    expect(source).toContain(':certificates="deploymentCertificateItems"')
