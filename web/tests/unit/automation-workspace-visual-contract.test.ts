...
describe('自动化工作台视觉合同', () => {
  it('复用共享组件，不在业务页面重建工作台壳层', () => {
    ...
    expect(sources.taskDrawer).toContain('grid-template-columns: minmax(0, 1fr) auto')
    expect(sources.taskDrawer).toContain('grid-column: 2')
    expect(sources.taskDrawer).toContain("['resourceName', 'displayName'")
    expect(sources.taskDrawer).toContain('normalizeDeploymentResourceName')
    expect(sources.taskDrawer).not.toContain('transform: translateY(-50%)')
