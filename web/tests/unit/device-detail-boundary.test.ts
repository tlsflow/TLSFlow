...
describe('统一设备详情动作边界', () => {
  ...
  it('设备列表提供插件资产编辑入口并复用声明式表单', () => {
    ...
    expect(devicesSource).toContain("label: t('assets.inventory.actions.edit')")
    expect(devicesSource).toContain("service_asset.manage")
    expect(devicesSource).toContain('<UnifiedAssetEditModal')
    expect(editSource).toContain('<GcPluginForm')
    expect(editSource).toContain('serviceAssetId')
    expect(editSource).toContain('updateServiceAsset')
  ...
  it('云服务详情只由统一设备详情模态框承载', () => {
    ...
    expect(devicesSource).not.toContain('isCloudServiceRow')
    expect(devicesSource).toContain('<UnifiedAssetDetailModal ref="assetDetailModal" />')
    expect(devicesSource).toContain('assetRefOf(row)')
    expect(modalSource).toContain('getManagedDevice')
    expect(modalSource).toContain('getServiceAssetDetail')
  ...
  it('统一详情沿用旧 Agent 视觉骨架且保留双列站点布局', () => {
    ...
    expect(modalSource).toContain("allowedActions.includes('cloud.service.discover')")
    expect(modalSource).toContain('discoverServiceAssetResources')
    expect(modalSource).toContain("rootType: 'SERVICE_ASSET'")
