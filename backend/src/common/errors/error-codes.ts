...
const registeredErrorCodes: Record<string, ErrorCodeDefinition> = {
  ...
  APPLICATION_EXECUTION_REFERENCE_INVALID: { httpStatus: 422, message: '应用执行引用无效' },
  APPLICATION_CURRENT_PLUGIN_UNAVAILABLE: { httpStatus: 422, message: '应用当前插件不可用' },
  APPLICATION_CURRENT_WORKFLOW_UNAVAILABLE: { httpStatus: 422, message: '应用当前工作流不可用' },
  APPLICATION_EXECUTION_UPDATE_REQUIRED: { httpStatus: 409, message: '应用执行配置需要更新' },
  APPLICATION_EXECUTION_UNSUPPORTED: { httpStatus: 422, message: '应用执行配置不受支持' },
  APPLICATION_COMPATIBILITY_STALE: { httpStatus: 409, message: '应用执行兼容性状态已过期' },
