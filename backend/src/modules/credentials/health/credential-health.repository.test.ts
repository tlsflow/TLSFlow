...
test('健康检测能够识别设备插件绑定中的凭据关联', async () => {
  ...
  try {
    ...
    await database.query(`insert into unified_plugin_versions
      ...
      values ('plugin-version-binding-health','tenant-binding-health','device.test','1.0.0','USER','WORKFLOW_DSL','BOTH','UNSIGNED','SELF_MANAGED','{}'::jsonb,'package','manifest','{}'::jsonb,'ENABLED','NOT_REQUIRED','[]'::jsonb,'{}'::jsonb,now(),now())`);
