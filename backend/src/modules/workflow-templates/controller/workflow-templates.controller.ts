...
import type { CreateWorkflowTemplateInput, UpdateWorkflowTemplateInput, WorkflowRuntimeInput, WorkflowStepRuntimeInput } from '../dto/workflow-templates.dto.js';
...
export class WorkflowTemplatesController {
  ...
  register(router: Router): void {
    ...
    router.post('/api/v1/workflow-template-runs/test-step', '执行单节点模拟运行', tag, async (request) => ({ statusCode: 200, body: await this.service.testStep(request.body as WorkflowStepRuntimeInput) }));
  }
...
export function getWorkflowTemplateRouteContracts(): RouteContract[] {
  ...
  return [
    ...
    { method: 'POST', path: '/api/v1/workflow-template-runs/test-step', operationId: 'testWorkflowTemplateStep', summary: '执行单节点模拟运行', tags: tag, responseSchema: objectSchema },
  ];
}
