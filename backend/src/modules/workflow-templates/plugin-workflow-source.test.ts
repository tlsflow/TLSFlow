...
test('插件工作流来源过滤、派生和内部只读形成闭环', async () => {
  ...
  const content={apiVersion:'gcac.workflow/v1' as const,kind:'CurlSshWorkflow' as const,metadata:{name:'plugin-deploy',version:'1.0.0'},variables:{},steps:[{name:'wait',type:'wait' as const,seconds:1}]};
