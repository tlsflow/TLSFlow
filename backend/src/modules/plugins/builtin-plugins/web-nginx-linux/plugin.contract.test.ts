...
const pluginId = 'web.nginx.linux' as const;

test(`${pluginId} 只接受 Linux Nginx PEM 并使用 reload`, () => {
  ...
  assert.equal(snapshot.paths.every((path) => path.startsWith('/opt/gcac/')), true);
});
