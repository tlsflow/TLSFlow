...
const PLUGIN_ID = 'cloud.huawei';
const PLUGIN_VERSION = '2.0.1';
const PROVIDER = 'huawei';
const SIGNATURE_ALGORITHM = 'HUAWEI-SDK-HMAC-SHA256';
...
// 工厂只在标准 Runner 子进程中执行；缺少主适配器注入的固定摘要时立即失败关闭。
...
async function execute(context, hostApi, descriptor) {
  ...
  try {
    ...
    const secret = await hostSecretResolve(hostApi, { grantId: requiredIdentifier(credential.grantId, 'credential.grantId'), secretRef: requiredSecretRef(credential.secretRef), purpose: `cloud.${operation}` }, refs);
    ...
    if (operation === 'deploy' || operation === 'rollback') {
      const artifact = await hostArtifactRead(hostApi, { grantId: requiredIdentifier(credential.grantId, 'credential.grantId'), artifactRef: requiredArtifactRef(input.certificateArtifactRef) }, refs);
...
function signRequest(service, secret, requestInput, body, security, operation) {
  ...
  const timestamp = huaweiTimestamp(requestInput.timestamp);
  ...
  const signature = hmacHex('sha256', requiredCredential(secret.secretKey, 'secretKey'), stringToSign);
  const authorization = `SDK-HMAC-SHA256 Access=${requiredCredential(secret.accessKey, 'accessKey')}, SignedHeaders=${signedHeaders}, Signature=${signature}`;
  return {
    provider: PLUGIN_ID,
    ...
    path: url.pathname,
    query: queryObject,
    headers: { authorization, host: url.host, 'content-type': 'application/json', 'x-sdk-date': timestamp, 'x-gcac-request-nonce': security.nonce },
...
function huaweiTimestamp(value) { if (typeof value !== 'string' || !Number.isFinite(Date.parse(value))) throw contractError('CLOUD_INPUT_INVALID', '请求时间戳无效'); return new Date(value).toISOString().replace(/[-:]/g, '').replace(/\.000Z$/, 'Z'); }
