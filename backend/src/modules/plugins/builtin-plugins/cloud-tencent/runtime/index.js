...
function signRequest(service, secret, requestInput, body, security, operation) {
  ...
  const scope = `${time.date}/${serviceName}/tc3_request`;
  const stringToSign = `TC3-HMAC-SHA256\n${time.seconds}\n${scope}\n${sha256Hex(canonicalRequest)}`;
  const secretDate = hmac('sha256', `TC3${requiredCredential(secret.secretKey, 'secretKey')}`, time.date);
  const secretService = hmac('sha256', secretDate, serviceName);
  const secretSigning = hmac('sha256', secretService, 'tc3_request');
  const signature = hmacHex('sha256', secretSigning, stringToSign);
  const authorization = `TC3-HMAC-SHA256 Credential=${requiredCredential(secret.secretId, 'secretId')}/${scope}, SignedHeaders=${signedHeaders}, Signature=${signature}`;
  return {
    provider: PLUGIN_ID,
    ...
    path: url.pathname,
    query: sanitizeQuery(requestInput.query),
    headers: { authorization, host: url.host, 'content-type': 'application/json', 'x-tc-action': action, 'x-gcac-request-nonce': security.nonce, 'x-tc-timestamp': time.seconds },
...
function failureResult(context, operation, error) {
  ...
  const code = typeof item.code === 'string' ? item.code : 'CLOUD_PLUGIN_FAILED';
  const mayBeUnknown = item.mayBeUnknown === true || context?.writeEffect === true && code === 'CLOUD_HOST_CALL_FAILED';
  return { success: false, status: context?.writeEffect && mayBeUnknown ? 'UNKNOWN' : 'FAILED', summary: { provider: PROVIDER, operation: operation ?? 'unknown' }, normalizedObjects: [], warnings: [], error: { code, message: typeof item.safeMessage === 'string' ? item.safeMessage : 'Cloud 插件执行失败', retryable: item.retryable === true, mayBeUnknown, secretRedacted: true } };
}
...
function tencentTimestamp(value) { if (typeof value !== 'string' || !Number.isFinite(Date.parse(value))) throw contractError('CLOUD_INPUT_INVALID', '请求时间戳无效'); const date = new Date(value); return { seconds: Math.floor(date.getTime() / 1000), date: date.toISOString().slice(0, 10) }; }
...
function hostError(safeMessage) { const error = contractError('CLOUD_HOST_CALL_FAILED', safeMessage); error.mayBeUnknown = true; error.retryable = true; return error; }
