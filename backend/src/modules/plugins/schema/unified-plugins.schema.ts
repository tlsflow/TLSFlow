...
export function validateUnifiedPluginManifest(input: unknown): UnifiedPluginManifestV1 {
  ...
  return {
    ...
    resources: {
      ...
      ...(resources.inputContracts === undefined
        ? {}
...
function readCompatibility(input: unknown): UnifiedPluginManifestV1['compatibility'] {
  ...
  assertKnownKeys(record, new Set(['productFamilies', 'frameworkTypes', 'targetTypes', 'managementMethods', 'executionLocations', 'artifactContracts', 'serviceEndpoints']), 'compatibility');
  return {
    ...
    serviceEndpoints: record.serviceEndpoints === undefined ? undefined : requireStringArray(record.serviceEndpoints, 'compatibility.serviceEndpoints'),
  };
}
