...
export class PluginBindingsRepository {
  ...
  async saveBinding(record: PluginBindingV1): Promise<PluginBindingV1> {
    await this.db.query(`insert into unified_plugin_bindings
      ...
      on conflict (id) do update set variable_bindings=excluded.variable_bindings,secret_bindings=excluded.secret_bindings,
      ...
      JSON.stringify(record.secretBindings), JSON.stringify(record.certificateArtifactBindings), JSON.stringify(record.connectionBindings),
...
interface BindingRow extends Record<string, unknown> {
  ...
  secret_bindings: Record<string, string>; certificate_artifact_bindings: PluginBindingV1['certificateArtifactBindings'];
