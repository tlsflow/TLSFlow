...
test('Projection 只根据 Contract 和 Resolver 输出生成统一分组', () => {
  ...
  const projection = new DeploymentInputProjectionService().project({ contract, resolvedInput: resolved });
...
test('Projection 不返回凭据 Secret 或敏感 Artifact 内容', () => {
  const projection = new DeploymentInputProjectionService().project({ contract: fixtureContract(), resolvedInput: fixtureResolved() });
