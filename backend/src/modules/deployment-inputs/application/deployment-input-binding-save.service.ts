...
export class DeploymentInputBindingSaveService {
  ...
  validate(request: ValidateDeploymentInputBindingSaveRequest): ValidatedDeploymentInputBindingSaveV1 {
    ...
    const current = request.currentAssetOverride?.pluginVersionId
      && request.currentAssetOverride.pluginVersionId === request.pluginVersionId
      ? filterSubmittedBindings(request.contract, request.currentAssetOverride.inputBindings ?? emptyInputBindingsV1()).inputBindings
