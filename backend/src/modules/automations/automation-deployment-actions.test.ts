...
test('证书计划动作只调用 DeploymentPlan 端口并传递快照目标', async () => {
  const calls: unknown[] = [];
  const service = new AutomationDeploymentActionService({ create: async (input) => { calls.push(input); return { id: 'plan_1' } as never; }, dryRun: async () => ({}), submit: async () => ({}) as never, execute: async () => ({}) });
  ...
  assert.deepEqual((calls[0] as { targets: unknown[] }).targets, [{ certificateBindingId: 'binding_1', serviceAssetId: 'asset_1' }]);
});
