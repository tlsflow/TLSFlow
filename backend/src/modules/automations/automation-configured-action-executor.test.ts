...
test('历史 executionOptions.dryRun 不再创建预检任务，直接提交并执行计划', async () => {
  const calls: string[] = [];
...
test('历史运行中的 Dry-run 不阻断自动化正式部署', async () => {
  const calls: string[] = [];
  const deployment = new AutomationDeploymentActionService({
...
test('历史 dryRunFirst 配置不再成为自动化执行门槛', async () => {
  const calls: string[] = [];
  const deployment = new AutomationDeploymentActionService({
  ...
  const result = await executor.execute({
    ...input,
    action: { type: 'execute_deployment_plan', position: 1, config: { source: 'created_by_previous_action', dryRunFirst: true } } as never,
  });
