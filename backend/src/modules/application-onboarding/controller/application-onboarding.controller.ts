...
export class ApplicationOnboardingController {
  ...
  private async selectCertificate(request: HttpRequest) {
    await this.assertWrite(request);
    const body = validateObject(request.body, { expectedStateVersion: { type: 'number', required: true }, certificateId: { type: 'string', required: true }, certificateVersionId: { type: 'string', required: true } });
