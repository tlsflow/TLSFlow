...
test('非受管资产只保存 WorkflowExecutionBinding 且拒绝插件 Assignment', async () => {
  ...
  const saved=await service.saveStandaloneWorkflowExecution(tenantId,asset.id,{workflowExecution:{tenantId,workflowTemplateId:'workflow_user',workflowVersionSelection:'LATEST_PUBLISHED',runner:'CONTROL_PLANE',inputBindings:{apiVersion:'gcac.input-bindings/v1' as const,connections:{},variables:{},credentials:{},artifacts:{}}}});
...
test('编辑资产基础信息时不重新校验历史无契约工作流绑定', async () => {
  ...
  const service=new ApplicationAssetExecutionService(db);
  const input={tenantId,workflowTemplateId:'workflow_legacy',workflowVersionSelection:'PINNED' as const,workflowVersionId:'workflow_legacy_v1',runner:'CONTROL_PLANE' as const,inputBindings:{apiVersion:'gcac.input-bindings/v1' as const,connections:{},variables:{},credentials:{},artifacts:{}}};
