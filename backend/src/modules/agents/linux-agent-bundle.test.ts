...
describe('Linux Agent 安装材料', () => {
  it('只返回固定版本、摘要、签名和受控 Artifact 引用', () => {
    ...
    assert.equal(material.version, '0.1.11');
    assert.match(material.artifactRef, /^artifact:\/\/gcac\/agents\/linux-go-full-agent\/0\.1\.11\//);
    assert.match(material.digest, /^[a-f0-9]{64}$/);
    assert.match(material.signature, /^artifact:\/\/gcac\/signatures\/agents\/linux-go-full-agent\/0\.1\.11\//);
  ...
  it('不同架构仍只能映射到预登记的固定 Artifact', () => {
    ...
    assert.equal(materials[0]?.version, '0.1.11');
