...
function renderLinuxBootstrapScript(manifest: unknown): string {
  ...
  return [
    ...
    '};',
    'const quarantineStaleTrustMaterial = (materialPath, trustedKeySet) => {',
    '  if (Object.keys(trustedKeySet || {}).length === 0 || !fs.existsSync(materialPath)) return;',
    '  let material;',
    '  try {',
    ...
    '    material = undefined;',
    '  }',
    '  const matchesTrustedKeySet = material',
    '    && sameKeySet(material.capabilityKeySet, trustedKeySet)',
    '    && sameKeySet(material.policyAuthorityKeySet, trustedKeySet);',
    '  if (matchesTrustedKeySet) return;',
    '  const backupPath = `${materialPath}.bak.${new Date().toISOString().replace(/[^0-9]/g, "")}.trust-root-rotated`;',
    '  fs.renameSync(materialPath, backupPath);',
    '  process.stderr.write(`已备份与本次安装信任根不一致的 Agent 授权材料：${backupPath}\\n`);',
    '};',
    'const config = {',
    ...
    '  // 旧材料不能用新安装固定的信任根验证时，必须先隔离，允许注册流程拉取新材料。',
    '  // 文件只改名备份；运行期仍然对新材料执行严格的 fail-closed 签名验证。',
    '  quarantineStaleTrustMaterial(config.authorizationMaterialPath, config.authorizationTrustKeySet);',
    '}',
