...
function renderWindowsCompatibilityBootstrapScript(manifest: unknown): string {
  ...
  return [
    ...
    "$programArgument = 'program=\"' + $agentTarget + '\"'",
    '$deleteOutput = (& netsh.exe advfirewall firewall delete rule name=all $programArgument 2>&1 | Out-String)',
    'if ($LASTEXITCODE -ne 0 -and $deleteOutput -notmatch "(?i)no rules match|没有规则匹配|找不到规则") { throw "Compatibility Agent firewall rule cleanup failed: $agentTarget`n$deleteOutput" }',
    ...
    '}',
    '[System.IO.File]::WriteAllText($configPath, $jsonSerializer.Serialize($config), $utf8NoBom)',
    ...
    '$metadata = @{ ServiceName = $serviceName; DisplayName = $displayName; InstallRoot = $installRoot; ConfigPath = $configPath; DataDir = $dataDir; LogDir = $logDir; BinaryPath = $agentTarget; InstalledAt = (Get-Date).ToString("o"); Mode = "windows-service-compatibility-bootstrap" }',
    '$metadataPath = Join-Path (Split-Path -Parent $configDir) "service.install.json"',
    '[System.IO.File]::WriteAllText($metadataPath, $jsonSerializer.Serialize($metadata), $utf8NoBom)',
