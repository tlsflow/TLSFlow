...
export function validateAgentExecutionReceipt(input: unknown): AgentExecutionReceiptV1 {
  ...
  exactKeys(value, ['receiptVersion', 'operationId', 'planId', 'planDigest', 'agentId', 'tenantId', 'tokenId', 'status', 'startedAt', 'completedAt', 'operationResults', 'nonceConsumed', 'errorCode', 'unknownReason', 'digest'], 'AgentExecutionReceiptV1');
  ...
  if (typeof value.nonceConsumed !== 'boolean') fail('nonceConsumed', '必须是布尔值'); if (value.status === 'UNKNOWN' && value.unknownReason === undefined) fail('unknownReason', 'UNKNOWN 回执必须说明原因'); if (value.status === 'SUCCESS' && value.errorCode !== undefined) fail('errorCode', 'SUCCESS 回执不能携带错误码'); if (value.errorCode !== undefined) identifier(value.errorCode, 'errorCode'); if (value.unknownReason !== undefined) nonEmptyString(value.unknownReason, 'unknownReason'); if (Date.parse(value.completedAt as string) < Date.parse(value.startedAt as string)) fail('completedAt', '完成时间不能早于开始时间'); digest(value.digest, 'digest'); if (computeAgentExecutionReceiptDigest(value as unknown as AgentExecutionReceiptV1) !== value.digest) fail('digest', '执行回执摘要与合同内容不匹配');
