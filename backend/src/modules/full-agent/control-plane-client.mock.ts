...
export class MockControlPlaneClient {
  ...
  register(config: FullAgentConfig, detected?: DetectedCapabilities): FullAgentIdentity {
    const registration = this.service.register(config.tenantId, {
    ...
    if (detected) {
      this.service.reportCapabilities(config.tenantId, {
  ...
  enqueueTask(identity: FullAgentIdentity, input: Omit<Parameters<AgentsApplicationService['enqueueTask']>[1], 'agentId'>): AgentTaskEnvelope {
    return this.service.enqueueTask(identity.tenantId, { agentId: identity.agentId, ...input }, this.nextRequestId());
  }

  pullTask(identity: FullAgentIdentity): AgentTaskEnvelope | undefined {
    return this.service.pullTasks(identity.tenantId, identity.agentId, 1)[0];
  }

  ack(identity: FullAgentIdentity, task: AgentTaskEnvelope, leaseId: string): AgentTaskEnvelope {
    return this.service.ackTask(identity.tenantId, { agentId: identity.agentId, taskId: task.id, leaseId });
  }

  log(identity: FullAgentIdentity, input: ControlPlaneTaskLogInput): AgentTaskLogEntry {
  ...
  result(identity: FullAgentIdentity, task: AgentTaskEnvelope, leaseId: string, result: StepExecutionResultLike): AgentTaskEnvelope {
  ...
  listLogs(identity: FullAgentIdentity, taskId: string): AgentTaskLogEntry[] {
