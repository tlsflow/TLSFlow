...
test('活动迁移目录只包含统一 baseline，空 PGlite 可直接建立当前结构', async () => {
  const files = (await readdir(activeMigrationDirectory)).filter((file) => file.endsWith('.sql')).sort();
  assert.deepEqual(files, [baselineFile]);
  ...
  try {
    ...
    const requiredTables = (await db.query<{ table_name: string }>(
      `select table_name from information_schema.tables
        ...
        order by table_name`,
      [['tenants', 'tenant_memberships', 'system_initialization_state', 'pg_documents', 'job_queue']],
    )).rows.map((row) => row.table_name);
    assert.deepEqual(requiredTables, ['job_queue', 'pg_documents', 'system_initialization_state', 'tenant_memberships', 'tenants']);
