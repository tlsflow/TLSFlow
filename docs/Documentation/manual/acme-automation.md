...
点击“新增”，填写域名、验证方式、提供商和账户，提交后在任务列表查看“排队中”“申请中”或“失败”等状态。成功后证书资产会出现新的证书版本；失败时先查看任务详情中的原因和凭据提示。

页面支持编辑、查看详情和删除。正在运行的任务不能删除；不要通过删除资产来中止续签。

## 凭据和事件

DNS Provider（DNS 服务商）凭据、EAB（外部账户绑定）密钥和账户密钥必须从“系统设置 → 凭据”引用。ACME 成功生成新版本后会产生证书新版本事件；“证书部署 → 自动化”可以按该事件创建部署计划。运行详情会固定这次事件对应的证书版本，不会在审批等待期间改用后来产生的版本。

## 本页截图占位符

> 【截图占位：ACME 提供商设置表单，显示目录地址、联系邮箱和账户信息】
>
> 【截图占位：新增申请任务表单，显示域名、挑战方式和申请状态】
