...
`AGENT_ATOMIC` plugins use signed Atomic Plans, and `TRUSTED_JS` plugins use the controlled plugin execution entry; neither becomes a workflow Step. User-plugin publisher cryptographic verification, unknown-code execution authorization, Trusted JS runtime gates, real vendor deployment/rollback, Gateway, and product-version acceptance remain `TODO` or `in_review` according to the capability.
