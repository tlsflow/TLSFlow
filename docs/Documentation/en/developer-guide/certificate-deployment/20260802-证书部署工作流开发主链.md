...
Publishing an `executionMode=declarative` plugin creates a read-only `plugin_internal` WorkflowVersion. Copying it creates a user-owned version with complete Provenance. Plugin upgrades do not rewrite user workflows; applying a new version is an explicit operation and requires new preflight.

`agent_plan` plugins use signed Agent v2 typed plans and are not Workflow sources. Code-bearing `isolated_process` capabilities are called only by an explicit DSL `plugin.action` Step through the separate Plugin Runner and terminal Action IPC contract (`gcac.plugin-runner/v2`; the v1 contract filename is retained as a historical path). The Runner handles that one Action and returns structured output; the DSL still owns deployment order, verification, checkpoints, and rollback. Runner, Policy Authority, real vendor deployment/rollback, Gateway, and product-version acceptance remain `TODO` or `in_review` according to the capability.
