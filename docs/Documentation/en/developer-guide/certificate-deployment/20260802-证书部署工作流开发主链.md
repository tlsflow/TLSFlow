...
`agent_plan` plugins use signed Agent v2 typed plans, and `isolated_process` plugins use the separate Plugin Runner and IPC v1; neither becomes a workflow Step. Runner, Policy Authority, real vendor deployment/rollback, Gateway, and product-version acceptance remain `TODO` or `in_review` according to the capability.
