...
准备 Linux 或 macOS 主机，并安装 Docker、Docker Compose v2 和 Buildx。主机需要能访问目标设备、证书服务以及你计划对接的外部厂商 API。
...
先按[部署参数](./deployment-parameters.md)把必填变量准备好，再按部署规模执行。标准版和单机版都需要 Agent Release Bundle（Agent 发布包）；它包含 Linux/Windows Agent、Windows Compatibility Agent 安装资源及 SHA-256 清单。先在仓库根目录执行：
