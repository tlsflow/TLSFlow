...
| `ca.certificate.query` | Query CA certificates and status | MEDIUM / read-only | `ca.operations.read` | `gcac.ca-certificate-query-input/v1` → `gcac.ca-certificate-query-result/v1` | None | Agent, control plane, Gateway |
| `ca.revocation.evidence` | Query certificate revocation evidence | MEDIUM / read-only | `ca.operations.read` | `gcac.ca-revocation-evidence-input/v1` → `gcac.ca-revocation-evidence-result/v1` | None | Agent, control plane, Gateway |
...
### 9.1 Environment, authentication, and common request rules

Replace `https://<gcac-host>` with the control-plane address. Control-plane APIs use JSON; every request except Logo downloads must send `Content-Type: application/json`. Log in with a local user to obtain a short-lived Bearer token:
...
The response is `{ "token": "…", "user": { … }, "permissions": [ … ] }`. Send `Authorization: Bearer <token>` on subsequent requests. Multi-tenant users must also send `X-Tenant-Id` (only a tenant returned by `/api/v1/tenants/accessible` is valid); a forged tenant header cannot be used for escalation. `X-Actor-Id` is retained only for compatibility test fixtures; production identity comes from the token.

Recommended headers:

| Header | When required | Rule |
| --- | --- | --- |
| `Authorization` | Every protected endpoint | `Bearer <token>`; expired tokens return `401 AUTH_UNAUTHENTICATED` |
| `X-Tenant-Id` | Token can access multiple tenants | Must be an accessible tenant; missing or stale context returns `403 TENANT_CONTEXT_INVALID`/`409 TENANT_CONTEXT_STALE` |
| `X-Request-Id` | Recommended | 1-128 letters, digits, `.`, `_`, `:`, `-`; echoed in the response for audit correlation |
| `X-Trace-Id` | Optional | Distributed-trace identifier; echoed or generated by the host |
| `X-Idempotency-Key` | Session creation and asynchronous/external writes | Stable within one tenant and endpoint; replaying the same request returns the original result, while a different payload returns `409 IDEMPOTENCY_CONFLICT` |

API timestamps are ISO 8601; convert them to browser-local time for display. Paginated responses use `{ items, page, pageSize, total }`. Never place tokens, Secrets, cookies, private keys, or Artifact contents in URLs, logs, or ordinary variables.

### 9.2 Plugin-version lifecycle: request and response fields

1. Read capability contracts and confirm `key`, `contractVersion`, permission, input/output schemas, and execution locations:
...
2. Import a version. `manifest` is a JSON object, and `resources` is required (package-relative paths mapped to text/JSON). `packageContent` is optional stable JSON used to calculate the package digest. It is not a Base64 archive and must not contain local paths; the resource map is always loaded from `resources`. The following snippet highlights the endpoint fields only: replace the abbreviated Manifest and Workflow resource with complete Schema-valid content before submitting; the placeholder string is not directly importable:
...
Success is HTTP `201`; the body is a PluginVersion containing at least `id`, `pluginId`, `version`, `status`, `packageSha256`, `manifestSha256`, `resourceSha256`, and `capabilities`. Persist the returned `id` and use it as `pluginVersionId` for all later calls. USER plugins are imported as `DISABLED + NOT_REQUIRED` and do not use permission approval. For BUILTIN or approval-required versions, approve only the declared permissions first:
...
Enable, disable, or retire with the same body `{ "pluginVersionId": "…" }`, using `/api/v1/plugin-versions/enable`, `disable`, or `retire`. A successful response is the updated PluginVersion; `enable` transitions to `ENABLED`, while `retire` is terminal and cannot be enabled again. After an import, approval, or enable failure, read `GET /api/v1/plugin-version-management/:pluginVersionId` and inspect `validationReport` instead of repeatedly submitting the same version.

### 9.3 Bindings and capability assignments

A Binding stores input references and managed context, never plaintext Secrets. `inputBindings` must include all four objects (send `{}` when empty):
...
The HTTP `200` body contains `{ id, tenantId, pluginVersionId, mode, inputBindings, managedContext, status, version, createdAt, updatedAt }`. Query with `GET /api/v1/plugin-bindings?bindingId=<id>`. Updates require optimistic locking:
...
Submit this to `PATCH /api/v1/plugin-bindings`; a version mismatch returns `409 RESOURCE_VERSION_CONFLICT`. Re-read and merge; never overwrite blindly.

Assign a capability only when `ownerType`/`ownerId` refer to an object visible in the current tenant and the Binding and PluginVersion belong to the same tenant/version:
...
The response contains `id`, `status`, timestamps, and the input fields. `precedence` is one of `DEVICE_DEFAULT`, `TARGET_OVERRIDE`, or `ASSET_OVERRIDE`; cloud services use `SERVICE_ASSET`. Resolve the effective source before execution:
...
`null` means no effective capability. A non-null result is the `CapabilityAssignment` record with its `id`, owner, `capabilityKey`, `pluginVersionId`, `pluginBindingId`, precedence, status, and timestamps. Then read the referenced PluginVersion management detail, Binding, and compatible-plugin endpoint to confirm PluginVersion is `ENABLED`, Binding is `ACTIVE`, and execution location and compatibility are valid.

### 9.4 Unified errors, status codes, and retries
...
Common statuses: `400 VALIDATION_FAILED` (invalid fields), `401 AUTH_UNAUTHENTICATED` (missing/expired token), `403 AUTH_FORBIDDEN` or `SEC_PERMISSION_DENIED` (permission/object access), `404 RESOURCE_NOT_FOUND`, `409 RESOURCE_ALREADY_EXISTS`/`RESOURCE_VERSION_CONFLICT`/`IDEMPOTENCY_CONFLICT` (conflicts), `422 PLUGIN_CONTRACT_INVALID` (invalid contract/resource), `429` (gateway rate limit), `500 SYSTEM_INTERNAL_ERROR`, `502 PLUGIN_OPERATION_FAILED`, `503` (target or Runner unavailable), and `504 EXECUTION_TIMEOUT`. `details` is redacted; never depend on Secrets in it.

Automatically retry only read requests where no external write could have occurred and the error is explicitly retryable. Reuse the same `X-Idempotency-Key` for an idempotent write. `UNKNOWN`, `202`, or `mayBeUnknown=true` means the external state may have changed; inspect execution records or read the target before retrying, and never replay with a new idempotency key blindly.

### 9.5 Minimal Runner IPC v2 implementation

The host starts Runner with a fixed absolute path. stdin/stdout use UTF-8 JSON Lines (one JSON object per line; logs must go to stderr, never stdout). Every object has `protocolVersion: "gcac.plugin-runner/v2"`, `messageType`, a unique `requestId`, and `sentAt`. Messages other than `hello` must reuse the host-fixed `pluginVersionId`, tenant, Action digest, and three resource hashes.

Minimal handshake and execution sequence:
...
When calling a Host API, Runner sends `host_call`, including the Action binding fields plus `method`, `input`, at least one `grantRefs`, `idempotencyKey`, `deadlineAt`, and `timeoutMs` (1-120000). The host returns `host_result` with the same `requestId`: success is `{ "ok": true, "output": { … } }`; failure is `{ "ok": false, "error": { "code", "message", "retryable", "mayBeUnknown", "secretRedacted": true } }`. Runner may call only the eight registered methods. Cancellation uses `cancel.targetRequestId` and `cancel_result`; health uses `ping/pong`; draining uses `shutdown/shutdown_result`.

The host rejects a handshake or execution if any of these differ: `pluginVersionId`, `pluginId`, `pluginVersion`, `capabilities`, `permissions`, `packageHash`, `manifestHash`, `resourceHash`, Action Contract version, or input/output schema digests. A Runner cannot execute two Actions concurrently. Timeout, crash, cancellation, or late results are converged by the host to `FAILED`, `UNKNOWN`, or `CANCELLED`.
...
### 12.5 Agent authorization API sequence

Plugins never issue Tokens, Policy Decisions, or local policies. After the control plane confirms the caller's `deployment.plan.execute` permission, it calls the Agent interfaces below. The plugin supplies only the plan declared in its Manifest and the computed `planDigest`:

1. **Generate authorization material**: `POST /api/v1/agents/:agentId/policy-provisioning`. The body must contain `pluginId`, `pluginVersionId`, `capability`, `planDigest`, `policyRef`, `policyVersion`, `actions`, `allowedPaths`, `allowedServices`, `commandRules`, `artifactDigests`, `lifetimeSeconds`, and the complete `compiledPlan`; `approvalRef` is optional. Keep `lifetimeSeconds` short and within the plan deadline. HTTP `201` returns the Policy Authority Token, Decision, Agent Local Policy, KeySet, and one-time Nonce. These objects are for this execution only and must not be persisted by the plugin.
2. **Queue execution**: `POST /api/v1/agents/tasks`. The body contains `agentId`, `executionRunId`, `executionStepId`, a stable `idempotencyKey`, and `payload`. `payload.actionType` is `agent.plan.validate` or `agent.plan.execute` and must carry the `agentPlan`, `token`, `policyDecision`, `localPolicy`, `keySet`, `nonce`, and Artifact digests for the same `planDigest`. Validation uses `writeEffect=false`; execution uses `writeEffect=true`. HTTP `201` returns the task object; do not mutate task state yourself.
3. **Read and converge**: use `GET /api/v1/agents/tasks?agentId=<id>` or the execution record to inspect task status. The Agent posts a signed Receipt to `POST /api/v1/agents/tasks/result`. The Receipt's `planDigest`, `tokenId`, `nonce`, `agentId`, and every `operationId` must match the original plan. Once `nonceConsumed=true`, the same plan cannot be submitted again; expiration, revocation, or signature mismatch converges to `FAILED`/`UNKNOWN`.

Minimal task request (authorization objects come from the previous response; values are placeholders):
...
Agent API authentication, tenant, and `X-Request-Id` rules are the same as section 9.1. Agent mTLS sessions and enrollment tokens belong to Agent installation/heartbeat protocols; plugins must not reuse or manage them.
