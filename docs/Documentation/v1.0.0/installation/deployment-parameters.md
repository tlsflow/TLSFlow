...
| `DOCS_VERSION` | `1.0.0` | Docker 构建参数，决定前端“使用手册”入口跳转的文档版本；不属于运行时部署参数 |

源码构建也可以使用同名语义的 `VITE_DOCS_VERSION`（例如 `1.0.0` 或 `v1.0.0`）。Docker 构建时通过
`--build-arg DOCS_VERSION=1.0.0` 传入；未设置时暂使用 `1.0.0`。该变量只控制入口链接，文档镜像仍会完整复制
`docs/Documentation/.vitepress/dist`，以保留版本切换和中英文页面。
