...
及插件数据，不需要手动创建宿主机目录。`-v` 选项不是容器启动的硬性要求，但生产环境必须保留，
否则删除或重建容器后数据、Token 密钥和运行时安全材料都会丢失。需要在 NAS 共享目录中直接管理文件或备份时，
使用下面“宿主机目录绑定”场景的命令。
...
在群晖、威联通或 Unraid 图形界面中，按同样的容器名、端口、两个环境变量和一个
`data` Bind Mount（绑定挂载）创建容器即可；该目录必须允许容器写入。镜像默认使用 UID/GID `10001:10001`，若 NAS 的共享文件夹启用严格
权限控制，应提前授予该 UID/GID 读写权限。

旧版本若使用 `/var/lib/gcac/pglite` 单独挂载，可以继续按旧命令运行；迁移到新的单目录布局时，先停止旧容器，
将旧 PGlite、工作流、运行时、TLS Inspector 和插件目录分别复制到 `DATA_ROOT/pglite`、`DATA_ROOT/workflows`、
`DATA_ROOT/runtime`、`DATA_ROOT/tls-inspector` 和 `DATA_ROOT/plugins`，再使用新的单目录命令启动。不要在运行中的 PGlite 目录上复制文件。
...
| `GCAC_PGLITE_DATA_DIR` | `/app/data/pglite` | 自定义 PGlite 数据目录；修改后必须同步调整对应 Bind Mount |
