...
export async function inspectTarget(target, options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      trustPathUnsupportedCount: riskSummary.trustPathUnsupportedCount,
...
function buildRiskSummary(protocols, cipherSuites, protocolDetails, trustPaths, simulations, sectionErrors) {
  ...
  return {
    ...
    hstsTooShort,
    trustPathIssueCount: trustPaths.filter((item) => ['untrusted', 'incomplete'].includes(item.status)).length,
    trustPathUnsupportedCount: trustPaths.filter((item) => item.status === 'unsupported').length,
